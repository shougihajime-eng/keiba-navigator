...
(function () {
  ...
  function renderWin5() {
    ...
    if (w5.wf && w5.wf.leakage_free && w5.wf.summary) {
      ...
      if (allZero) {
        // 🚨 2026-08-12: ここに「金の3連単 (期待 +129%)」「短距離・実力派 (期待 +37%)」と
        //   手書きの成績が焼き付いていた（毎日の見張り役が発見）。
        //   実際の成績は value_tan3 が回収率 0%・52戦0勝、value_uren_short が 49.5% で、
        //   どちらも「✕止める」判定。**この2つを「本気の戦略」として勧めるのは嘘。**
        //   数字は書かず、本物の成績を見てもらう案内だけにする。
        wfBanner.appendChild(el("div", { class: "wf-warn", html:
          "<b>正直な話</b>: AI でも 5 レース連続で当てるのは とても低い確率です。" +
          "何十日 試して 0 回でも不思議ではありません。<br>" +
          "WIN5 は<b>遊びで小額だけ買う</b>馬券です（払戻率70%＝ふだんの馬券より不利）。" +
          "ほかの買い方の<b>実際の成績</b>は、画面の「本当の成績」を見てください。"
        }));
      }
    ...
    // 2026-08-12: WIN5 の正直な但し書き。ここだけ見て買われると危ないので必ず出す。
    body.appendChild(el("div", { class: "win5-honest", html:
      `⚠ <b>WIN5 は当たりません。</b>このアプリで過去 34 日ぶんを検証したところ、`
      + `3 つの戦略すべてで <b>的中 0 回・回収率 0%</b> でした。`
      + `5 レース連続で当てる確率はもともと 0.1% ほどです。<br>`
      + `お金の見込み（期待いくら）は<b>出しません</b>。`
      + `人気馬でそろえれば当たりやすい代わりに配当は小さく、穴でそろえれば逆になるため、`
      + `「確率 × 平均配当」で計算した数字は本物になりません。<br>`
      + `娯楽として少額で楽しむ馬券です。本気で狙うものではありません。` }));
    ...
    strategyList.forEach((s) => {
      ...
      // 🚨 2026-08-12 削除: ここに「期待 ¥12,682 / EV ×63.4」と出していたが**成り立たない数字**。
      //   計算は「当たる確率 × 平均配当800万円」だが、**800万円は“平均的な当たり組合せ”の配当**。
      //   人気馬でそろえれば当たりやすい代わりに配当は桁違いに小さく、穴でそろえれば逆。
      //   高い確率と大きい配当を掛け合わせた、ありえない値（200円に対して63倍）だった。
      //   しかもこのアプリ自身の検証は **34日中0回的中・3戦略とも回収率0%**。
      //   → お金の見込みは出さない。点数・費用・当たる見込みだけを出す。
      c.appendChild(el("div", { class: "sub" },
        st.hitProb != null ? `当たる見込み ${(st.hitProb * 100).toFixed(2)}%` : "—"));
      // クリック → カスタム編集起動
