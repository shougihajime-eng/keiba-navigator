...
(function () {
  ...
  function renderRaceHub(mountEl, ctx) {
    ...
    function renderOdds(hist) {
      ...
      // 🚨 2026-08-12: /api/odds-history が返す「出来上がりのグラフ」があればそれを出す。
      //   ここは元々 配列 hist / hist.history を待っていたが、API はその形を返さない。
      //   そのため **データがあるのに ずっと「記録はまだありません」** と出ていた（実機で発覚）。
      //   グラフは lib/odds_history.js が作る（サーバー側）。画面側で描き直さない＝作りを2つ持たない。
      if (hist && hist.ok && typeof hist.svg === "string" && hist.svg.indexOf("<svg") === 0) {
        const c = hist.counts || {};
        h += `<div class="rh-chart">${hist.svg}</div>`;
        if (c.prePost != null) {
          h += `<p class="rh-note">発走前に ${c.prePost} 回ぶんのオッズを記録しています`
            + (c.plotted != null ? `（${c.plotted} 回ぶんを描いています）` : "") + `。</p>`;
        }
        h += `</section>`;
        return h;
      }
