...
function parseLine(line) {
  if (!line) return null;
  // ★Wave9-fix: 入力サイズ上限。1 行 500 文字超は不正入力として弾く (DoS 対策)
  if (typeof line !== "string" || line.length > 500) return null;
  const cols = line
    .replace(/　/g, " ")
    .split(/[,\t\s]+/)
    .map(s => s.trim())
    .filter(Boolean)
    .slice(0, 20);  // 1 行あたり最大 20 トークン (それ以上は不正入力)
...
function parseTextInput(text) {
  ...
  // ★Wave9-fix: 全体サイズ上限 (DoS 対策・100KB)
  if (text.length > 100_000) return [];
  return text.split(/\r?\n/)
    .slice(0, 100)                                  // 最大 100 行
    .map(l => l.trim())
