...
//
// 【偽プレースホルダー結果の検出】(必殺１ごうてい側で起きた事故の予防)
//   JV-Link / Supabase race_results は「結果確定前」でも race_id 行が作られる仕様で、
//   results: [] / payouts: {} の空状態の行が存在しうる。
//   旧実装はこれを finalize すると winnerEntry=undefined で won=false / payout=0 を
//   返してしまい、profit = -amount の「偽の外れ」を bets に書き込んでいた。
//   競艇アプリ(必殺１ごうてい)では同パターンで545件の偽外れデータが蓄積した実績あり。
//   → results 配列が空 もしくは 主要券種の払戻が一切ない場合は未確定として扱う。
function isResultUnsettled(result) {
  if (!result) return true;
  if (!Array.isArray(result.results) || result.results.length === 0) return true;
  // 1着の rank 行が無ければ未確定
  const hasRank1 = result.results.some(r => r && Number(r.rank) === 1);
  if (!hasRank1) return true;
  const p = result.payouts || {};
  // 主要券種(単勝/馬連/3連単/3連複/複勝)の払戻情報が一切ないなら未確定
  const hasAny =
    (p.tan   && typeof p.tan.amount   === "number" && p.tan.amount   > 0) ||
    (p.uren  && typeof p.uren.amount  === "number" && p.uren.amount  > 0) ||
    (p.tan3  && typeof p.tan3.amount  === "number" && p.tan3.amount  > 0) ||
    (p.fuku3 && typeof p.fuku3.amount === "number" && p.fuku3.amount > 0) ||
    (Array.isArray(p.fuku) && p.fuku.some(f => f && f.amount > 0));
  return !hasAny;
}

function finalizeBet(bet, result) {
  if (!bet || !result || !Array.isArray(result.results)) return null;
  // ★偽プレースホルダー判定: 結果が空のまま finalize しない
  if (isResultUnsettled(result)) return null;
