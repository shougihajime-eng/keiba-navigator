...
function buildWin5(races, options = {}) {
  ...
  // 各戦略の当選確率と組合せ数を計算
  // ★Wave9-fix: 確率積を log-domain で計算 (5 戦の確率積は 1e-8 オーダーで丸め誤差消失する)
  function computeStrategy(topK) {
    let logProbSum = 0;      // log domain で積み上げ
    let combo = 1;           // 組合せ数 (=点数)
    let broken = false;
    const picksPerRace = [];
    for (const r of perRace) {
      if (!r.ok) { broken = true; break; }
      const horses = r.ranked.slice(0, topK).filter(h => h && Number.isFinite(h.prob));
      if (horses.length === 0) { broken = true; break; }
    ...
    const probSum = broken ? 0 : Math.exp(logProbSum);
    ...
    return {
      ...
      hitProb: probSum,
      hitProbPct: (probSum * 100).toFixed(probSum < 0.001 ? 6 : 4) + "%",
      expectedReturn: ev,
      evRatio,
      picksPerRace,
    };
