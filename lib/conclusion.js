...
function buildConclusion(race) {
  ...
  const biasInfo = describeBias(enriched);
  const biasAdjusted = prediction.horses.map(p => {
    const horseObj = enriched.horses.find(h => h.number === p.number) || {};
    const adj = computeBiasAdjustment(enriched, horseObj);
    return { ...p, _adj: adj.adjustment, prob: (p.prob ?? 0) * adj.adjustment };
  });
  const sumAdj = biasAdjusted.reduce((a, x) => a + (x.prob || 0), 0);
  const adjustedHorses = sumAdj > 0
    ? biasAdjusted.map(p => ({ ...p, prob: p.prob / sumAdj }))
    : biasAdjusted;
