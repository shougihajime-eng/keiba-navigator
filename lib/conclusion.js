...
const EX = require("./exotic"); // ディスカウントHarville(連系・3連系の的中率)

function isMissing(v) { return v === null || v === undefined || v === "" || (typeof v === "number" && Number.isNaN(v)); }

// ★2026-06-28 EVスケール改定 (市場アンカー+実測較正) に合わせ、買い/狙える判定を健全な水準へ。
//   旧値(RACE_GO 1.30 等)は壊れた過大EV前提で高すぎ、正直なEVでは誰も届かず常時「見送り」になった。
const TH = {
  STRONG_BUY:    1.12,
  BUY:           1.00,
  WEAK:          0.70,
...
function calWinRate(p, bins) {
  // ★単調ルックアップ (app.js と同方式)。bins は toFixed で丸められ隣が連続しないため、
  //   範囲一致 (pMin<=p<=pMax) だと“すきま”に落ちた値が最終 return で最大ビンに化ける重大バグになる。
  //   「p 以下を最初に満たすビン」を拾えば すきまでも正しく最寄り上側ビンになる。
  if (p <= bins[0].pMax) return bins[0].win;
  for (const b of bins) { if (p <= b.pMax) return b.win; }
  return bins[bins.length - 1].win;
}
// 複勝(3着内)率: 較正表の in3(過去の実測3着内率)を、市場de-vig勝率 p で引く。
// in3 が無い古い表のときは null(=複勝率は出さない)。
function calPlaceRate(p, bins) {
  if (!bins.length || bins[0].in3 == null) return null;
  if (p <= bins[0].pMax) return bins[0].in3;
  for (const b of bins) { if (p <= b.pMax) return b.in3 ?? null; }
  return bins[bins.length - 1].in3 ?? null;
}

// ─── 連系・3連系の「正直な的中率」(ディスカウントHarville＋実測較正) ──────
// build-exotic-calibration.cjs が作った exotic_calibration.json で
// 予想確率(Harville)→実測的中率 に写像する(楽観バイアス除去)。
let _EXCAL = null, _EXCAL_TRIED = false;
function loadExoticCal() {
  if (_EXCAL_TRIED) return _EXCAL;
  _EXCAL_TRIED = true;
  try {
    const c = require("../data/jv_cache/exotic_calibration.json");
    if (c && c.wide && c.trio) {
      _EXCAL = c;
      if (c.discount) EX.setDiscount(c.discount.lam, c.discount.tau);
    }
  } catch { _EXCAL = null; }
  return _EXCAL;
}
// ビン表(予想p→実測rate)を単調ルックアップ。表が無ければ生の予想を返す(安全)。
function calExotic(p, bins) {
  if (!Array.isArray(bins) || !bins.length) return p;
  if (p <= bins[0].pMax) return bins[0].rate;
  for (const b of bins) { if (p <= b.pMax) return b.rate; }
  return bins[bins.length - 1].rate;
}
// しきい値 (環境変数で調整可。0 も有効値として受けるため Number.isFinite で判定)
function envNum(name, def) { const v = Number(process.env[name]); return Number.isFinite(v) ? v : def; }
const ODDS_CAP = envNum("KEIBA_ODDS_CAP", 50);   // この倍率超は本命に選ばない (大穴は構造的に-EV)
// EDGE_CAP: 較正勝率は「市場de-vig勝率 × EDGE_CAP」まで。
//   ここを大きくすると本命を過大評価し“ほぼ全レース買い”の偽シグナルになる。
//   控除(overround≈1.25)を超えて+EVと言い切るのは本当に強い根拠がある馬だけにすべきなので、
//   控えめ(1.15)を既定にする＝堅実側。本当に市場よりズレた馬だけが EV>1 になる。
const EDGE_CAP = envNum("KEIBA_EDGE_CAP", 1.25);
...
function buildConclusion(race) {
  ...
  // ─── 真の勝率を作る: 市場de-vig → 実測較正 → モデルで控えめに調整 ───────
  //   ★設計(2026-06-28 市場アンカー方式):
  //     ① mkt = 市場de-vig勝率(単勝オッズ 1/odds をレース内で正規化)。
  //     ② calBase = 実測較正(place_calibration)で mkt を「実際に勝った率」へ写像。
  //        ＝過去3796レースの本物の実績。較正表は“市場de-vig勝率”を入力に作ってあるので、
  //          ここでも mkt を入力にする(入力と表の尺度を一致させる＝較正の鉄則)。
  //     ③ モデル(ensemble系)の手相診断ぶんだけ控えめに上下に傾ける(tilt)。
  //        モデルは弱いので tilt は [1/TILT_CAP, TILT_CAP] に厳しく制限し、暴れさせない。
  //     ④ 較正勝率は市場implの EDGE_CAP 倍までに抑える(乖離しすぎ防止)。
  //   これで「大穴で爆発するEV」も「本命の過大評価」も起きず、+EVは本当に市場が
  //   過小評価している馬だけに出る。
  const calBins = loadCalibration();
  const TILT_CAP = 1.6; // モデルが市場baseを動かせる上限/下限の倍率(弱いモデルの暴走防止)
  ...
  // ① 市場de-vig → ② 実測較正 → ③ モデルtilt を掛けた“生の真勝率”を作り、レース内で正規化
  let calSum = 0;
  const calRaw = {};
  for (const h of enriched.horses) {
    ...
    // ベース = 較正済み市場(市場が無い馬はモデル生確率にフォールバック)
    let base;
    if (Number.isFinite(m) && m > 0 && calBins) base = calWinRate(m, calBins);
    else base = (pr ?? 0);
    // モデルの手相診断ぶんを tilt として控えめに反映(market基準の相対意見)
    if (Number.isFinite(m) && m > 0 && pr != null) {
      let tilt = pr / m;
      if (!Number.isFinite(tilt) || tilt <= 0) tilt = 1;
      tilt = Math.max(1 / TILT_CAP, Math.min(TILT_CAP, tilt));
      base *= tilt;
    }
    calRaw[h.number] = base; calSum += base;
  }
  ...
  const horsesWithEv = enriched.horses.map(h => {
    ...
    // 真勝率 (正規化) → エッジ上限 (市場implの EDGE_CAP 倍まで)
    let prob = rawProb;
    if (calBins && calSum > 0) {
    ...
    // 複勝(3着内)率: 市場de-vig勝率 m から較正表の実測3着内率を引く(過去の本物の率)。
    const mForPlace = mkt[h.number];
    const place = (calBins && Number.isFinite(mForPlace) && mForPlace > 0)
      ? calPlaceRate(mForPlace, calBins) : null;
    ...
    return {
      ...
      prob, rawProb, place, ev: evSafe, buyEligible,
  ...
  // 本命・買い候補は「買える(ODDS_CAP以下=大穴でない)」馬だけから選ぶ。
  // ★大穴へのフォールバックは禁止: eligible が空なら買い目ゼロ＝正直に「見送り」にする
  //   (全馬が大穴の難解戦で-EVの大穴を本命に推さないため)。
  const eligible = evHorses.filter(h => h.buyEligible);
  const sorted = [...eligible].sort((a, b) => b.ev - a.ev);
  ...
  // 2026-08-12: prediction.name が取れず「使用モデル: undefined v4.0.0」と画面に出ていた。
  //   分からないものを undefined と出さない（分からないなら そう書く）。
  const _mdlName = prediction.name || prediction.source || (prediction.model && prediction.model.name) || null;
  const _mdlVer  = prediction.version || null;
  const _mdlTxt  = _mdlName ? `${_mdlName}${_mdlVer ? " v" + _mdlVer : ""}` : "（名前が取れていません）";
  reasonList.push(`使用モデル: ${_mdlTxt} / 信頼度 ${(conf*100).toFixed(0)}% (${confidenceLabel})`);
  reasonList.push("⚠ ヒューリスティックモデルの推定値です。JV-Link接続後に学習モデルへ差替予定。");

  // ★2026-06-28 正直な予想ツール化:
  //   バックテスト(過去3796レース)で「単勝の+EV買い」は控除20%により構造的に成立せず
  //   どの基準でも回収率マイナスと判明(scripts/backtest-ev-gate.cjs)。
  //   よってアプリの主役を「儲かる買い目(幻)」から「正直な予想=勝ちそうな本命(勝率順)」へ。
  //   本命は EV(=勝率×オッズ) ではなく 推定勝率の高い順 に選ぶ(=実際に勝ちそうな馬)。
  const byProb = [...evHorses].sort((a, b) => (b.prob ?? 0) - (a.prob ?? 0));
  const ROLE = ["honmei", "taikou", "tanana"];
  const ROLE_LABEL = ["◎本命", "○対抗", "▲単穴"];
  const forecastPicks = byProb.slice(0, 3).map((h, i) => {
    const winPct = Number.isFinite(h.prob) ? (h.prob * 100).toFixed(0) : "—";
    return {
      number: h.number, name: h.name, odds: h.odds, popularity: h.popularity,
      prob: h.prob, place: h.place, ev: h.ev, grade: evGrade(h.ev), role: ROLE[i],
      note: `${ROLE_LABEL[i]}・推定勝率 ${winPct}%`,
      isValue: Number.isFinite(h.ev) && h.ev >= TH.BUY, // 参考: 期待値プラスか(稀)
      jockey: h.jockey || null, trainer: h.trainer || null,
    };
  });
  const top3Buy = forecastPicks;          // 主役=予想(勝率順)。downstream の topPick も本命になる。
  const top3Avoid = avoid.slice(0, 3).map(h => mkPick(h, "avoid"));

  // ★人が見る結論文(verdictTitle/Reason)は「予想(本命)」を主役に正直化する。
  //   内部 enum の verdict(pass/go/neutral) は他処理のため残すが、表示文だけ予想ベースに上書き。
  const fav = forecastPicks[0];
  if (fav) {
    const favPct = Number.isFinite(fav.prob) ? (fav.prob * 100).toFixed(0) : "—";
    verdictTitle = `本命 ${fav.number} ${fav.name || ""}`.trim();
    verdictReason = `いちばん勝ちそうなのは ${fav.number}番 ${fav.name || ""}(推定勝率 ${favPct}%)。`
      + `馬券は払戻しが約80%(控除20%)のため、買い続ければ長い目ではマイナス＝これは予想であって儲け保証ではありません。`;
  }

  // ★連系・3連系の「正直な的中率」を計算(ディスカウントHarville＋実測較正)。
  //   本命-対抗ワイド、上位3頭BOX 3連複 が「だいたい何回に1回当たるか」を本物の数字で出す。
  const exoticCal = loadExoticCal();
  let exotic = null;
  try {
    const pm = {};
    for (const h of evHorses) if (Number.isFinite(h.prob) && h.prob > 0) pm[h.number] = h.prob;
    const rk = EX.rankByProb(pm);
    const a = rk[0], b = rk[1], c = rk[2];
    if (a != null && b != null && c != null && Object.keys(pm).length >= 4) {
      const wideP = EX.pWide(pm, a, b);
      const trioP = EX.pTrio(pm, a, b, c);
      const wideRate = exoticCal ? calExotic(wideP, exoticCal.wide) : wideP;
      const trioRate = exoticCal ? calExotic(trioP, exoticCal.trio) : trioP;
      exotic = {
        wide: { pair: [a, b], rate: wideRate },        // 本命-対抗ワイド(2頭が3着以内)
        trio: { combo: [a, b, c], rate: trioRate },    // 上位3頭BOX 3連複(3頭で1-2-3着)
        calibrated: !!exoticCal,
      };
    }
  } catch { exotic = null; }
  ...
  return {
    ...
    picks: top3Buy,
    exotic,                                  // ★連系・3連系の正直な的中率(ディスカウントHarville＋較正)
    avoid: top3Avoid,
    // 全出走馬を勝率順で (WIN5 等が中位馬も含めて広く流すのに使う・APIには載せない内部用)
    allHorses: [...evHorses].sort((a, b) => (b.prob ?? 0) - (a.prob ?? 0))
      .map(h => ({ number: h.number, name: h.name, prob: h.prob, ev: h.ev, odds: h.odds, popularity: h.popularity })),
