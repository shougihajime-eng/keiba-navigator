...
function mkPick(h, role) {
  let note;
  if      (role === "buy" && h.ev >= TH.STRONG_BUY) note = `強い買い (EV ${h.ev.toFixed(2)})`;
  else if (role === "buy")                          note = `買い候補 (EV ${h.ev.toFixed(2)})`;
  else if (role === "avoid")                        note = `期待値マイナス (EV ${h.ev.toFixed(2)})`;
  else if (role === "overpopular")                  note = `${h.popularity}番人気だがEV ${h.ev.toFixed(2)} — 過剰人気`;
  else if (role === "undervalued")                  note = `${h.popularity}番人気だがEV ${h.ev.toFixed(2)} — 過小評価`;
  return {
    number: h.number, name: h.name,
    odds: h.odds, popularity: h.popularity,
    prob: h.prob, ev: h.ev, role, note,
  };
}
...
function buildConclusion(race) {
  ...
  const horsesWithEv = enriched.horses.map(h => {
    ...
    return {
      ...
      prob, ev,
      sex_age: h.sex_age || null, jockey: h.jockey || null,
    };
  });
  ...
  const reasonList = [];
  if (sorted[0]) reasonList.push(`EV最大: ${sorted[0].number} ${sorted[0].name || ""} — 推定勝率 ${(sorted[0].prob*100).toFixed(1)}% × オッズ ${fmt(sorted[0].odds)}倍 = EV ${sorted[0].ev.toFixed(2)}`);
...
module.exports = { buildConclusion };
