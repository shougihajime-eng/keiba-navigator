...
- **📌 本当に血統を効かせる唯一の道＝血統データの拡充**（JV-Link 過去10年で `pedigreeSurfaceAff` を厚くする）。今は血統データが薄く多くの馬で中立(1.0)になる＝重みだけ上げても効かない・むしろノイズ。**データが本丸**（[[keiba-ev-market-anchor-and-guards]] の“真の上積みはデータ”と一致）。⚠これは非開催日の別作業。本人が「血統を強くして」と言ったら、まずこのデータ拡充から。
