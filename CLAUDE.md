...
- **🔍 全画面の操作チェック + 死にボタン修正 (2026-05-31・本人「クリック/バグ/操作性/スマホ対応を全部確認して、修正箇所があれば直して」指示)** — web v2 の全部品(BlockA-C/RaceCard/各モーダル/資金管理/実験室/折りたたみ群など)を1つずつ精読。ビルド・型チェックともエラーゼロ、スマホの土台(viewport=拡大可)も問題なしを確認。**唯一見つかった不具合=「直近の反省」カードの「全反省履歴を見る」ボタンが onClick 未設定で押しても無反応(死にボタン)**。押すと「反省ダッシュボード」折りたたみが自動で開いてそこまでスクロールするよう修正(`Collapsible` に `openOn` を追加し `keiba:open-section` イベントで外部から開ける仕組みに)。ビルド成功 → commit 9be9e9f push 済(本番v2へ自動反映)

- **📦 「硬い事実シグナルを日付つきで貯める箱」を新設 (2026-05-30・本人「レース前の情報・騎手の発言を取り込めば的中率上がるのでは?」相談 → 選択肢から「2=効きそうな硬い情報だけ先に」+「1=貯める箱を作る」の両方を選択)** —
  - **方針の結論**: ニュースや騎手コメントを「そのまま予想に足す」のは効きにくい (公開情報はオッズが既に織り込む=このアプリ自身が証明済[[keiba-no-positive-ev-current-data]])。世の中の「ニュース解析で的中97%」は実験室が見破った過適合/リークと同種。逆に「賢いお金の動き(短時間のオッズ急落)/取消/馬体重の大きな増減/装備変更(ブリンカー初装着)」のような“事実の変化”は人が見落としがちで効く可能性。ただし過去ログが無いと検証不能 → **「今から日付つきで生ログを貯める箱」を用意**(10年データ収集と同じ“育つ”作戦)
  - **調査で判明**: 取消(`ijyou_code`)も装備(`blinker`)も実はパース済だった。足りないのは時間を追う履歴。既存 `lib/odds_movement.js` は `/api/odds-movement` を叩いた時だけ動き本番では `/tmp` で消える=実質貯まっていなかった
  - **新規 `jv_bridge/archive_signals.py`**: 毎時取得(`fetch_diff_hourly.ps1`)の build_all 直後に、開催日が今日前後(前1日〜後2日)のレースだけを対象に、各馬のオッズ・人気・取消区分・馬体重・増減・ブリンカー + 馬場・天気を時刻つきで `data/jv_cache/signals/<raceId>.json` に追記。**変化が無ければ追記しない(冪等)・空の出走表シェル(馬番0)は除外・古い確定レースは対象外**(昔のレース3500件を誤って全保存→開催日フィルタで48件=今日分のみに修正)
  - **build_race_json.py**: 出走表に `blinker`(ブリンカー)を追加(装備変更=硬い事実)
  - **.gitignore**: `!data/jv_cache/signals/` を許可リストに追加 → 毎時の `git add -A` で自動的に貯まり続ける
  - **正直方針**: ここでは一切「判断」しない・予想には混ぜない。事実を残すだけ。数ヶ月後に貯まったら別途 `validate_signals.py`(未作成)でリークなしに「本当に効くか」採点 → 効くものだけ採用
  - **検証**: archive_signals 自己テストOK / 実データで48件保存・2回目は0追記48 unchanged(冪等確認) / node smoke 128/0 / pytest(build_result_json)13通過。**commit/push 済**
  - **【続き】画面表示 + 採点役を追加 (同日・選択肢から「1=画面に出す」+「2=採点役を先に用意」の両方)**:
    - **画面に「事実メモ(参考)」を表示**: `build_race_card.py` が各レースに `facts`(賢いお金の動き/取消/馬体重大増減/装備変更)を付与 → `RaceCard.tsx` に色分けバッジ + 「まだ検証していないので予想には混ぜていない」と正直注記。馬体重差は壊れデータ対策で 16〜40kg のみ採用(古い5/24に+304kg等の異常値があり除外)
    - **採点役 `jv_bridge/validate_signals.py` 新規**: signals と results を突き合わせ、各事実が出た馬の3着内率/1着率を全体平均と比較。サンプル<30は「データ不足・あとN頭必要」と正直表示(現状0件=全部データ不足)。リークなし(事実は発走前情報・結果と突合するだけ)
    - **毎時取得に build_race_card + validate_signals を追加** → 事実メモと採点が自動更新(育つ)
    - **.gitignore**: `signals_validation.json` も許可
    - **検証**: web `npm run build` 成功(型OK) / 36レース中18レースに事実メモ生成 / node smoke 128/0。**commit/push 済(本番v2へ自動反映)**
  - **【続き2】実機確認 + 事実タイプ追加 (同日・選択肢から「1=今週末(実開催日)に確認」+「2=事実タイプを増やす」の両方)**:
    - **実機確認(5/30土)**: `KeibaDiffFetch-0900〜1430` が今日も全部 Result=0(成功)で自動稼働を確認。**ただし今日のsignalはオッズが0件**=毎時取得(option=2 diff)は“前売りオッズ”を取っていないため「賢いお金の動き(money_in/out)」はまだ貯まらない。次の一手はオッズ(O1/RT)取得を毎時に足すこと
    - **共有部品 `jv_bridge/race_facts.py` 新規**: 事実の定義を1箇所に集約(表示と採点が必ず同じ定義を使う)。build_race_card と validate_signals 両方がこれを import
    - **事実タイプ追加**: 連闘/休み明け/距離変更 を実装。ただし**休み明け・距離変更は当面オフ(`TRUST_FULL_HISTORY=False`)**。理由=過去約8ヶ月ぶんしかデータが無く“本当の前走”を取り違える(実際は3週間前なのに『112日ぶり』と誤判定→53件も誤発生)ため。**連闘だけは欠損でも誤検知しない(見つけた前走が7日以内=本物)ので採用**。本当の前走は JRA-VAN の馬毎レース記録(UM)を繋いだら True にする
    - **検証**: カード再生成で事実 36件(連闘8/取消17/馬体重11)=誤りなし / web build 成功 / node smoke 128/0。**commit/push 済**

- **🚤→🐎 必殺一号艇の予想思想を移植・全レースを4段階で判定 (2026-05-30・本人「必殺一号艇のオッズ計算/予想の仕方を参考に・利益を出したい・全部予想して買う/買わないが見る人に分かりやすく」指示)** — 競艇アプリ「必殺一号艇」で効いていた3本柱を競馬の実測データに合わせて移植。本人は選択肢から「両方いっぺんに」を選択:
  - **① 期待値の冷まし (`jv_bridge/honest_ev.py` 新規)**: 必殺のev-calibration思想。生EV(AI勝率×オッズ)を「過去に実際そういう馬を買って戻ってきた割合」に冷ます。検証(value_ev_result.json/8ヶ月3593R)の実回収率 約0.67〜0.81 に合わせ、オッズ帯ごとの冷まし倍率(中オッズ0.74-0.76/大穴70倍超0.42=穴馬ほど強く)。→ 競馬では市場が賢く正直EVはほぼ1.00未満=お買い得は基本ない、を数字で正直に表示
  - **② 構造フィルタ + 4段階判定**: 必殺の「強制見送り」を競馬化。過剰人気(本命オッズ<1.8=配当安すぎ)/上位横並び(本命勝率<0.13 or 2番手との差<0.015=荒れ)を機械的に見送り。残りを 絶好機/勝負/様子見/見送り に分類 + 馬番入り一言理由 + おすすめの買い方(複勝)
  - **しきい値を実データで較正**: 過去10開催360Rで「本命勝率0.20+→実際3着内70%・1着39%」「0.10未満→37%」と判明。選別は本物(必殺の選別力と同じ)。検証で 絶好機68.9%/勝負65.2%/様子見60.7%/見送り52.0% と段階通り本命3着内率が下がることを確認
  - **③ 見やすい表示**: 全レースカード(`RaceCard.tsx`)を4段階色分けに刷新(上部に件数サマリ・各レースに色バッジ+一言理由・展開表に「正直EV」列)。`rating.ts`/`BlockA`/`StarRating`の語彙も「絶好機/勝負/様子見/見送り」で統一・冷ましEVで一貫化
  - **正直な前提を明示**: 競馬は市場効率的で+EVは存在しない(検証済[[keiba-no-positive-ev-current-data]])。利益保証ではなく「ムダな赤字を減らす選別」と「一目で買う/見送りが分かる見やすさ」が価値、とアプリ内コピーに明記
  - **検証**: `npm run build` エラーなし・honest_ev自己テストOK → commit 0a8ddac push済(本番v2へ自動反映)。注: race_card_latest は実データ+予想が鋭い直近日(5/17)でデモ生成。実開催日には自動で最新に上書きされる
  - **④ 複勝のおすすめ金額 (続けて本人「2」選択)**: 必殺の「金額まで出す」を移植。資金管理の予算連動で 絶好機=予算3%/勝負=その7割・100円単位に固定(`fukushoStake` in bankroll.ts)。儲けでなく損を抑える小額と明記。RaceCard/BlockA両方に表示 (commit 9c4f8aa)
  - **⑤ 見た目を磨く (本人「1」選択)**: 「買うなら候補(絶好機/勝負)」を金縁goldPulse(絶好機)/深緑(勝負)の大きめカードで2列強調・複勝金額を大きく・様子見/見送りはコンパクト一覧に。BlockAの本日カードも段階バッジ+複勝大箱で統一 (commit 30e18da, 6e8c487)
  - **⑥ 実機QA (本人「2」選択)**: Chromeヘッドレスでスマホ390px/PC1280pxを撮影し、崩れ/横はみ出し無し・読みやすさOKを確認。平日は「配信待ち」が正しく出ることも確認
  - **⑦ 開催日に自動更新 (本人「1」選択で発見・修正)**: build_race_card が自動パイプラインに入っておらず、画面の全レースカードが更新されない穴を発見。`race_day_pipeline.py` に build_race_card ステップ追加 + git push対象に race_card_latest.json 追加 + build_race_card の日付自動選択を「最新の本物データ(オッズ+予想)がある開催日」に改良(従来は出走前の空枠日を選んでいた) → 開催日にオッズが届くと今日の実データで自動更新 (commit 5ede577)
  - **⑧ 資金管理の使い勝手 (本人「2」選択)**: 予算を 5千/1万/2万/3万/自分で の**ワンタップ選択**に(非エンジニア向け)・「この予算だと複勝の目安: 絶好機¥X/勝負¥Y」を表示しレースカードの金額と連動を可視化 (commit 92e8002)
  - **⑨ 成績の見える化 (本人「1」選択)**: BlockC直下に PerformanceInsights 新設(記録時のみ表示)。的中率+月別収支グラフ(プラス=緑上向き/マイナス=赤下向き)+段階別成績(factors.rating別の的中率/回収率)。store.tsに monthlyPL/tierBreakdown 追加 (commit 4c4bacc)
  - **⑩ 最終QA (本人「2」選択)**: ビルド/型OK・本番API全200・実機スクショ全セクション点検。軽微修正(createdAt欠落防御/払戻金の全角受理+0円誤確定防止/Map初期化堅牢化) (commit b0643e6)
  - **⑪ 🆕 買う金額を本人の固定ルールに (2026-05-30・本人が金額決定)**: **勝負=5,000円 / 絶好機=1万〜1万5千円(おすすめ1.5万) / WIN5=金額決めず自信のあるときだけ**。予算連動(3%)をやめ `bankroll.ts` の STAKE_PLAN 固定額へ。RaceCard/BlockA/BankrollCard/BetConfirmModal/WIN5 の金額表示と説明を統一 (commit 317dc5e)。**※今後この金額が本人の標準ルール**

- **🧪 自己成長する「実験モード(実験室)」を新設・本番公開 (2026-05-30・本人「世の中の成功例を学習して試す検証モードが欲しい・使うほど成長するアプリにして」指示)** —
  - **世の中の成功例を調査**: Qiita/note 10件超を読み、共通の勝ちパターン=「単勝にしぼる/人気・オッズをAIに教えない(=既存nopopと同じ)/自信レースだけ強くしぼる/オッズ高めを狙う/期待値で選ぶ」を抽出。記事の回収率120〜237%は主にシミュレーション(過去への過適合 or リーク)と判断
  - **成功例を現データで一次調査** (`jv_bridge/experiment_tansho_backtest.py`): リーク有り(未来を見た予想)だと単勝で291〜5540%の夢の数字が出るが、リークなし本物検証では同じ単勝作戦が68〜86%=負け。記事の高ROIの正体がリーク/過適合だと実証
  - **🌱 自己成長エンジン** (`jv_bridge/experiment_engine.py`): リークなしの per-bet キャッシュ value_ev_bets.json(41,780件・3期間・各期間を過去だけで学習)を読み、9作戦で「紙の上で買ったつもり」→本物の結果で自己採点→回収率順に自動ランキング→成長ログ(experiment_history.json)に snapshot 追記。データが増えるたび再評価され数字が育つループ
  - **正直な現状(リークなし)**: 最良でも「単勝・お得な中穴(期待値1.2/5-50倍)」で回収率81.1%。100%超は無し・全作戦勝期間0/3 → headlineで「まだ勝てる買い方は見つかっていません」と正直表示
  - **API**: `/api/experiment-status` `/api/experiment-history` を server.js + api/[...slug].js + .gitignore 許可リストに追加
  - **web v2**: ホームに `ExperimentLab.tsx` ブロック追加(作戦ランキング・正直な回収率の色分け・紙上収支スパークライン・「紙の上の実験/お金は使わない」明示)。JRA公式風ライトテーマに統一。本物機能(本日/予想/収支/反省)は一切不変
  - **検証**: `npm run build` 成功(型チェック通過)。**commit 88e7921 (push 済→本番v2自動反映)**
  - **データ更新で成長させる手順**: 週末データ追加後に `py -3.12 jv_bridge/walk_forward_value_ev.py`(リークなし再学習・~数分) → `py -3.12 jv_bridge/experiment_engine.py`(再採点) → commit/push で実験室が成長

- **👁️ 「見にくい」を解消・JRA 公式風に読みやすく全面調整 (2026-05-29・本人「JRA公式みたいに・文字も見やすく・今すごい見にくい」指示 → 参考: https://www.jra.go.jp/keiba/thisweek/)** — 白×緑化の後でも本人には見にくく、原因を「地色がうっすら緑がかり/背景に色付きの光と粒模様/文字が細い」と特定。読みやすさを 6 回に分けて強化:
  - **globals.css**: 地色を緑がかった白→ニュートラル薄グレー(#F4F5F6)・背景の色付き光(青/金/緑)とグレイン撤去で上端に緑のひと刷毛だけ・本文15→16px/行間1.6→1.75/字間0・文字色を全体に濃く(ink-faint #9AA99D→#6E776F)・JRA グリーン #0E8C4A→#0B7A40・動く sheen 反射を停止
  - **主要カード+折りたたみを全部一段大きく**: BlockA(今日の絶好機)/RaceCard(全レース予想と詳細表 text-xs→sm・極小10-11px撤去)/BlockC(収支)/BankrollCard(資金管理)/HonestStatus(正直な現在地)/CollapsibleSections(騎手ランキング・達成バッジ・戦略成績・凡例)/Collapsible 見出し
  - **検証**: `npm run build` 6回ともエラーなし → commit d9de6fa/6b85734/98d868f/3a0da51/82452ec/e0be9c9 等を push 済(本番 v2 へ自動反映)。色は1か所のトークン管理なので将来テーマ変更も簡単
- **🐎 背景に「ターフを走る馬」+「名馬列伝」の帯を追加・本番公開 (2026-05-29・本人「背景に名馬がいるとテンション上がる」)** —
  - `web/src/components/TurfBackdrop.tsx`: 全ページの奥(z-0)に、芝を疾走する競走馬(騎手付き)を薄い緑で3頭ゆっくり流す固定背景。文字の上には被らず読みやすさは維持。
  - `web/src/components/LegendStrip.tsx`: ヒーロー下に「名馬列伝」18頭(シンザン〜イクイノックス・名前+年+一行エピソード)が右から左へ流れる帯。ホバーで一時停止。
  - `web/src/components/icons/GallopHorse.tsx`: 手描きの走る馬シルエット(**写真は不使用=著作権セーフ**)。
  - reduced-motion 設定時は全アニメ停止。`npm run build` 成功。**commit 1a3f0c2 (push 済)**
  - 注: 名馬の「写真」は撮影者/JRAに著作権があるため使えない。今後ビジュアルを足すなら手描きイラスト or 名前・記録(事実)で構成する。

- **⏰ 自動スケジュールの見直し (2026-05-29・本人と相談して調整)** —
  - **毎時取得を「9:00 始まり」に前倒し**: `KeibaDiffFetch-0900` を追加 (毎日09:00・既存の09:30〜20:30と同設定)。朝イチのレースにも最新オッズで予想が間に合うように。`scripts/register_diff_hourly.ps1` の `$HOURS` にも 09:00 を追記済 (再登録で消えない)
...
- **💴 資金管理・規律カード(BankrollCard) + 馬ごとの「正直な期待値(EV)」列 を追加・本番公開 (2026-05-26・「100点への一手(C)損を抑える規律 +(A)馬ごとEV表示」を実装)** —
  ...
  - **全レース予想テーブルに「期待値」列 (`web/src/components/blocks/RaceCard.tsx`)**: 各馬の ev(=AI勝率×オッズ=100円が平均で何円戻るかの理論値)を展開テーブルに追加。1.00超は緑表示だが、「穴馬ほど市場に買われ過ぎ・単発の1.00超はノイズで継続して勝てる買い方ではない」と正直に注記 (favorite-longshot bias の警告)。誤って高EV馬を買わせない設計。テーブル min-width を 360→420px に
  - **検証**: `npm run build` 成功 / BankrollCard・RaceCard とも共通カラートークン経由なので「夜のターフ×金」ダークテーマに自動で馴染む
  - **注**: 残りの「100点への一手」は (B) 新コンポーネントのデザイン微調整 / (D) 過去10年データの自動リトライ待ち (rc=-501継続中)

- **🎨🌙 「夜のターフ×金」高級カジノ風ダークテーマへ全面リニューアル + 本番公開 (2026-05-26・ユーザー「世界一のデザイナーとして10点→100点・デザインが最優先」指示)** — 3案提示 → ユーザーが「夜のターフ・高級カジノ風(深い黒緑+金が宝石のように灯る)」を選択。実機Chromeで撮影確認しながら全面実装:
  - **globals.css 全面刷新**: キャンバスを深い黒緑(#080D0A)・金(#E8C26C)を主役にした配色トークン・背景に光の層(ターフ緑光/金スポット/暗がりのビネット)+グレイン・影を金グローへ・新アニメ(goldPulse/twinkle/floaty/sheen光スイープ/riseIn)・text-gold-grad/num-glow ユーティリティ
  - **ヘッダ**: 最上部の金輝線・暗いガラス(canvas/75 backdrop-blur)・LIVE 緑ドット
  - **勝負レースカード**: 金縁+ringグロー+表面の光スイープ・本命馬番を金グラデの玉に
  - **開催なしの日(月曜など)を生きた画面に**: 「今日は休む日」金グラデ+走る馬(浮遊)+次の開催までのカウントダウン(日/時間の金枠)+次開催日。空っぽの寂しい画面を解消
  - **収支**: 累計回収率を巨大金グラデ文字+0→実値のカウントアップ+100%ライン付きゲージ(100%超で金・未満でローズ)
  - **UI部品**: Button(gold/rubyは暗文字on-gold・金グラデ)/Card(暗面+上端ハイライト+goldグロー)/StarRating(金きらめき・暗い空星)/BetConfirmModal/RaceDetailModal(覆いを黒scrim・本命を金玉・focusを金線)
  - **検証**: `npm run build` 成功・実機Chromeで `/` と `/design` を撮影し夜テーマ+金の描画を確認・ベタ書きの明るい色は実質ゼロ(全部 共通カラートークン経由)
  - **本番反映**: web/src のみ commit (作りかけの分析スクリプトは除外) → `git push origin main` (commit 25f1a03) → keiba-navigator-v2.vercel.app に自動デプロイ

- **🖱️ 新アプリ(v2) のクリック性・操作性・スマホ/PC対応の最終QA + 修正 (2026-05-26・ユーザー「世界一厳しい検査員として10点→100点」指示)** — 実機をChromeで撮影+puppeteerで計測しながら全数検査し、操作系の不具合を根治:
  - **死んでいた「詳細」ボタンを根治**: BlockAの詳細ボタンが onClick 無しで無反応だった → 新規 `RaceDetailModal.tsx` (本命/対抗/3着のAI予想・期待値/信頼度/頭数・根拠・JRA公式リンク・そのまま「これ買う」へ) を作成して接続。見送りレース行(SkipRow)もタップで詳細が開くように
  - **ヘッダ日付が「作った日」で固まる不具合を根治**: page.tsx はサーバ静的生成なので `new Date()` がビルド時刻で凍結 (本番で「5月23日」のまま表示) → 新規 `LiveDate.tsx` (クライアントで実日付・1分ごと+復帰時に更新) に置換
  - **スマホでピンチ拡大できない問題**: layout.tsx の `maximumScale:1` を撤廃 → `maximumScale:5 / userScalable:true` (文字を大きく見たい人向け)
  - **「これ買う」モーダルが小画面で下切れ**: `max-h-[92dvh] overflow-y-auto overscroll-contain` でスクロール可に
  - **折りたたみの中身が仮文字だった件を実データ化**: WIN5=実3戦略(堅め/中波/万舟)描画+未開催は明示 / 自動化ステータス=`/api/automation-status` の実状態(予想計算・データ取得・AI学習の経過時間+信号色) / 達成バッジ=ローカル記録から8種を自動解除 / 騎手・厩舎ランキングは正直な「準備中」表示に
  - **PC余白が広すぎ** → `max-w-3xl → max-w-4xl`。**スマホ通知バーの文言**を狭幅で短縮表示
  - **横はみ出しはゼロと実測確認** (360/390/1280px で scrollWidth==clientWidth)。ビルドのCSS欠落騒ぎは「2つのビルドが同じ .next を奪い合った汚染」と判明 (クリーンビルドでCSS健全)
  - **注**: 見た目は「夜のターフ×金」ダークテーマへ全面刷新し、上記の操作系修正と統合のうえ **本番公開済** (2026-05-26 / commit 25f1a03)

- **🔬 「儲け最優先」で価値投資EVをズルなし検証 → 現データでは+EV戦略は存在しないと確定 (2026-05-26・ユーザー「お金をかけるので本気で稼げるアプリに・100点を目指せ」指示)** —
  - **ユーザーが方向選択**: 提示した3択(①正直な規律アプリ ②儲け最優先で挑戦 ③まず見た目)で **②儲け最優先** を選択 (「出ない可能性も高い」を承知の上)。
  - **今のAIの致命的弱点を特定**: 既存戦略は `walk_forward_v2` を読むと「AIが自信を持った馬を、オッズ(配当の良し悪し)を一切見ずに買う」だけ。`odds:null/ev:null` のまま発火。投資の本質「補正済み勝率×本物オッズ=期待値」を見ていなかった。
  - **5/25のオッズ修正で初めて可能になったEV検証を実施**: 過去レースの単勝オッズ充足率 92.9%(馬)/96.8%(レース) を確認。
  - **新規 `jv_bridge/walk_forward_value_ev.py`**: 各期間で過去のみnopop学習 + 過去のみIsotonic補正 + 「補正済み勝率×本物オッズ=EV」で価値投資。`jv_bridge/analyze_value_ev.py` でキャッシュ(41,780ベット)をしらみつぶしスライス。
  - **🚨 結論 (3手法すべて一致・回収率100%超は無し)**:
    - 単勝/複勝の価値投資 全EV閾値 → 60〜92% (人気1番ベタ買い77%にも勝てない)
    - 人気帯×オッズ帯×確信度×市場食い違いを総当たり → 100%超は「単勝モデルtop1×オッズ10-20倍 101.5%(勝期間2/3・249件)」等ノイズ級のみ
    - キャリブレーション確認: AIの補正済み勝率は正確(予想20%→実際20.6%)・市場も正確・**穴馬は市場が買われ過ぎ(暗示5%→実際1.3% = favorite-longshot bias)** で特に損
  - **確定事実**: AIの確率は正確・市場も効率的 → 控除率20-25%を超える持続的エッジは現8ヶ月3636Rには無い。プロジェクト自身の WF_v2 値とも完全一致。memory `keiba-no-positive-ev-current-data` に記録。
  - **唯一残った打ち手**: 過去10年データ取得 (約15倍)。`py -3.12-32 jv_bridge/jv_fetch.py aggregate --dataspec RACE --fromtime 20140101000000 --option 4` を起動 (JVInit OK・ダウンロード中)。ただし市場効率性ゆえデータ増でも+EV発見は不確実。
  - **ユーザー指示で本番公開・アプリ全体を Claude が引き取り**: 検証結果を報告 → ユーザーは「公開して・アプリ全部任せる」を選択。
  - **🆕 web v2 に 2 セクション追加・本番公開済 (commit 188af55)**:
    - `HonestStatus.tsx`: 「AIの正直な現在地」パネル。「必ず勝てる買い方は無い/AIの予想は正確/穴馬は罠」をデータ付きで明示しユーザーを誤った高ROIから守る。新・暗色金デザインに自然になじむ (意味別トークン使用)。
