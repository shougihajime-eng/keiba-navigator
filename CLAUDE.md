...
## 🧬 本人が重視する予想の要素＝血統・騎手・厩舎（2026-07-21 本人指示・恒久方針）

本人「**競馬で大事なのは血統。騎手・厩舎も大事**」。＝この3つを予想でしっかり効かせる方針。
- **現状（調査済）**：3つとも既に `predictors/ensemble_v1.js` の弱学習器に入っている＝**pedigree_fit（血統×距離馬場）／jockey_trainer（騎手・調教師の勝率）／career_form（騎手/厩舎の複勝率）**。ただし重みは控えめ（`computeWeights`：血統 pedigree=0.05〜0.10＝最小／jockey=0.10〜0.25／career=0.05〜0.15／odds=0.50→0.20 が最大＝市場アンカー）。
- ⚠**正直な注意（[[keiba-ev-market-anchor-and-guards]]／[[gambling-prediction-cannot-beat-takeout]]）**：市場オッズは**既に血統・騎手・厩舎を織り込んでいる**（賭ける人みんなが知っている）。だから“**無茶苦茶に**”盛ると二重カウントで**大穴を過大評価して逆に当たらなくなる**（過去に「ニセ強化」として血統/調教をはじいた実績あり）。＝本人の狙いは正しいが、やり方は「盛る」ではなく「**正しく効かせる**」。
- ✅**正しいやり方（恒久手順）**：重みを上げる → **`node scripts/backtest-honmei.cjs --last 1500`** で本命的中率を実測 → 市場一番人気より当てているか確認 → **良くなった／悪化しない分だけ採用・悪化したら戻す**。血統データ(pedigreeSurfaceAff)が薄い馬は中立(1.0)になる＝**データ拡充(JV-Link 過去10年取得)が本丸**（薄いまま重みだけ上げても効かない・むしろノイズ）。
- **結果（2026-07-21 検証済・重みは元のまま据え置き）**：`computeWeights` の血統/騎手/厩舎を **①強く上げた版(pedigree 0.14/jockey 0.16/career 0.10)** と **②ほどよく上げた版(0.09/0.13/0.08)** の2通りを `backtest-honmei --last 1500` で検証 → **どちらも本命的中は 31.4%（元と同じ）で横ばい・市場との差はむしろ広がった(-0.62pt→ ①-0.62 / ②-0.76)＝“盛るほど気持ち悪化”**。市場オッズが既に血統/騎手/厩舎を織り込むため“もっと効かせても当たりは上がらない”を実データで確認。→ **元の重み(pedigree 0.05/jockey 0.10/career 0.05)に戻した**（当たりが一番良いのは元。3要素は元々ちゃんと使われている）。控え＝`predictors/ensemble_v1.js.bak-20260721`（＝元と同じ）。smoke 128/0 合格。
- 🚨🚨 **【2026-08-12 決着】血統データを 99.9% までそろえたが、お金では効かなかった＝本番に繋がない**。
  下の「データが本丸」は**やり切った**（父が分かる馬 57.5%→**99.9%**・`build_sire_map.py` の3つのバグを修正）。
  そのうえで**AUCと回収率の両方**で測った結果が正反対だった：
  | | AUC（当てる力） | **お金（回収率）** |
  |---|---|---|
  | 血統を足すと | +0.0010・**5期間中5期間で改善** | **−1.98pt・5期間中1期間しか改善せず** |
  | 本命的中率 | — | 21.99% → **21.89%（下がった）** |
  95%の幅 −5.81pt〜+1.51pt＝**0をまたぐ**（運の範囲）。→ **繋がない**。
  ⚠**市場の1番人気の方が強い**（単勝 76.78% vs モデル 73.25%／本命的中 32.22% vs 21.99%）＝
  血統は既にオッズに織り込まれている、という [[keiba-ev-market-anchor-and-guards]] の結論がまた再現された。
  検証＝`jv_bridge/experiment_sire_roi.py`（リークなし・レース単位ブートストラップ・特徴の作り方は
  `experiment_sire_feature.py` の関数をそのまま import＝実装を2つ持たない）。
  📌**教訓の再確認＝AUCが上がっても お金は増えない。本番に繋ぐ判断は必ず回収率でする。**
  📌**血統データ自体は資産として残る**（99.9%そろっている）。「父の産駒勝率」という粗い使い方が効かなかっただけで、
  血統×距離・血統×馬場・母父などの別の使い方は まだ試していない。
- 🚨🚨 **【2026-08-12 続き・決着その2】血統の「きめ細かい使い方」を10通り試した → 全部ダメ。本番に繋がない**。
  上の「まだ試していない」を **やり切った**（父×距離／父×馬場／父×馬場状態／**母父(BMS)**／勝率でなく複勝率／レース内の相対値／全部盛り）。
  検証＝`jv_bridge/experiment_pedigree_map.py`（血統マップ）＋`…_pedigree_features.py`（特徴）＋`…_pedigree_roi.py`（判定）。
  **3,047レース・5期間・リークなし・レース単位ブートストラップ・試した数ぶんの補正(max-T)つき**。
  | | 複勝回収 | ベースとの差 | 95%の幅 |
  |---|---|---|---|
  | ⓪ 血統なし（今のモデル） | 79.06% | — | — |
  | いちばん良く見えた ⑥父×馬場状態 | 80.87% | **+1.81pt** | **−0.53 〜 +4.23（0をまたぐ）** |
  | ものさし 市場の1番人気の複勝ベタ買い | **83.98%** | — | — |
  **10通り中プラスに見えたのは8通りだが、95%の幅は全部0をまたぐ＝運の範囲**。試した数ぶんの補正は ±3.88pt で、それも超えていない。
  🚨🚨 **決め手＝ニセ血統での検査（`--placebo`）**：血統をシャッフルしたデタラメのデータで同じことをやると、
  **AUCは ±0.0002 に潰れる（＝リークは無い）のに、お金は ニセでも 複勝+1.76pt・単勝+8.03pt の「改善」が出た**。
  ＝本物で見えた「+1〜2pt」は **まぐれと見分けがつかない**。**3,047レースでは 複勝で±2.5pt、単勝で±5〜17pt より小さい差は測れない**。
  📌**「血統に情報が無い」のではない**（ここが大事）。モデルを通さない生の数字で5等分すると、
  父×馬場の複勝率が低い帯→高い帯で **実際の勝率が 4.17% → 11.34%（2.7倍）** ときれいに並ぶ＝血統は本物の情報。
  **ところが オッズも同じだけ短くなる**（市場が見込む勝率 5.61% → 13.77%）＝**その情報はもう値段に入っている**。
  だから **どの帯を丸ごと買っても 単勝66〜81%・複勝66〜74%** で、100%には遠い。[[keiba-ev-market-anchor-and-guards]] の再現。
  📌**AUCとお金は別物、をまた確認**＝AUCが一番上がった⑧（+0.0032）の複勝は+0.77pt、お金が一番上がった⑥（+1.81pt）のAUCは+0.0018。**順番が一致しない**。
  📌**単勝の「大当たり1回抜き」が効いた**＝⑦母父は単勝+4.45ptに見えたが、**¥25,290の万馬券1本を抜くと −1.48pt に反転**（＝平均の罠）。⑩も+10.88→+4.95。
  🔧**ついでに見つけた本物のデータ不具合（本番は未修正・実験側だけ直した）**＝`build_sire_map.py` の繁殖登録番号の直し方がまちがっている。
  8桁→10桁は「**あたま3文字のうしろに00を入れる**」（`11202242`→`1120002242`）が正しく、実データ**11,508頭で照合・食い違い0**で確認した。
  ところが `_norm_hansyoku()` は「**うしろに00を足す**」（→`1120224200`）。そのせいで SK(産駒マスタ)から取った父と UM(競走馬マスタ)から取った父が
  **同じ種牡馬なのに別IDになり、1頭の成績が2つに割れていた**（実測 父ID 1,499種類 → 直すと 1,140種類＝約3割が幽霊）。
  直すと 父・**母父とも 99.9%**（レースに出た12,793頭中12,774頭）そろう。⚠ただし**直しても お金は変わらなかった**（②の列）ので、
  本番の `sire_map.json` は**触っていない**（血統は本番未配線なので実害ゼロ。将来 血統を使うことがあれば ここを直してから使う）。
  📌**結論＝血統はもう探さなくてよい**。粗い使い方も きめ細かい使い方も 母父も、お金では動かなかった。次に賭けるべきは血統以外（下の「次の一手」）。
- 🚨🚨 **【2026-08-12 決着その2】調教（追い切り）も お金では効かなかった＝本番に繋がない**。
  2026-06-23 の判定は **AUC でしか測っておらず**（3特徴で −0.0001〜−0.0006）、お金では一度も測っていなかった。
  8/12 に索引（`data/jv_cache/chokyo/index.json`・`chokyo_full.json`）ができたので、血統と**同じ土台**で測り直した。
  **6通り**試した（①本数 ②最終追いからの間隔 ③坂路/ウッド・栗東/美浦 ④タイムの偏差 ⑤休み明け×本数 ⑥全部のせ）。
  | | 単勝回収 | ベースとの差 | 95%の幅 |
  |---|---|---|---|
  | ものさし＝市場1番人気 | 76.78%（複勝 83.98%） | — | — |
  | ベース（調教なし・nopop） | 73.25%（複勝 79.06%） | — | — |
  | 一番よかった「最終追い間隔」 | 77.54% | **+4.29pt** | **−2.63〜+13.04pt（0をまたぐ）** |
  | 一番わるかった「休み明け×本数」 | 73.67% | +0.42pt | −2.30〜+3.14pt |
  **見かけの +3〜4pt は全部ニセモノ**と3つの検査で確定：
  ①**大当たりを3つ抜くと差が消える**（本数 +3.43pt → **−0.01pt**）＝2〜3レースの万馬券で持ち上がっていただけ。
  ②**調教が一番そろっている484R（ほぼ全頭にデータ）では逆に下がる**（本数 −7.48pt・全部のせ −4.05pt）。
   同じく覆い率92%の期間6も全パターンがベース割れ。＝**データが濃いところほど効かない**＝効いていない証拠。
  ③**プラセボ検査**（調教の数字を他の馬とすり替えたニセデータで同じことをやる）でも
   **−5.62〜+6.72pt** 動いた＝本物の差 6通りとも この幅の中。
  さらに **選ぶ馬が変わった453Rでは的中率が 22.5%→16.6% に落ちている**（回収率だけ上がるのは
  「人気薄に乗り換えて たまたま当たった」形＝典型的な宝くじ買い）。
  検証＝`jv_bridge/experiment_chokyo_roi.py`（特徴は `experiment_chokyo_feature.py` の関数をそのまま import＝実装を2つ持たない）。
  **⚠ 正直な限界＝調教データの覆い率が期間でバラバラ**（期間1は13.7%・期間6は92.1%）。
  生データは 2025-08 からの差分取得ぶんしかなく、古い月ほど薄い。
  **10年ぶんのセットアップ取得（GUIクリックが要る）ができたら、覆い率をそろえて測り直す価値はある**。
  📌**教訓＝「AUCで否定した」は「お金で否定した」ではない。逆に「お金で+3ptだった」も、
  大当たり抜き・データが濃い部分・プラセボ の3つに通してから言う。**
- **📌〔達成ずみ〕本当に血統を効かせる唯一の道＝血統データの拡充**（JV-Link 過去10年で `pedigreeSurfaceAff` を厚くする）。今は血統データが薄く多くの馬で中立(1.0)になる＝重みだけ上げても効かない・むしろノイズ。**データが本丸**（[[keiba-ev-market-anchor-and-guards]] の“真の上積みはデータ”と一致）。⚠これは非開催日の別作業。本人が「血統を強くして」と言ったら、まずこのデータ拡充から。
- **🧬 血統データ取得の“壁”＝セットアップはダイアログが要る（2026-07-21 実地で判明）**：現状 `data/jv_cache` の血統データは **horse_master.json 177頭のみ・sire_map.json は存在しない**＝ほぼ空（＝`pedigreeSurfaceAff` がほぼ全馬 null＝血統が効きようがない）。取得を実地検証した結果：
  - **フル取得（過去分）＝`aggregate --dataspec BLOD --option 4`（セットアップ）は JV-Link の“セットアップ画面(GUIダイアログ)”が開きクリック待ちでハングする**（裏起動＝窓ゼロでは誰も押せず固まる。プロセスがJV-Link COMを握るので**放置は本番のライブ取得をふさぐ＝危険→即kill**）。実地で `python(32bit) jv_fetch.py aggregate --dataspec BLOD --fromtime 20140101000000 --option 4` が「セットアップ」窓を出して5分無反応→安全終了させCOM解放を確認。
  - **差分取得＝`--option 1`（ダイアログ無し）は裏で動くが `JVOpen rc=-1 (該当データなし)`**＝過去の血統は差分では取れない（JRA-VANは古い血統をセットアップ側でしか配らない）。
  - ✅**正しい取得手順＝本人が対話で1回だけ実行**：`! cd C:\Users\shoug\競馬 && "C:\Users\shoug\AppData\Local\Programs\Python\Python312-32\python.exe" jv_bridge\jv_fetch.py aggregate --dataspec BLOD --fromtime 20140101000000 --option 4` を**本人が実行→出てくるJV-Linkセットアップ窓をクリックしてDL完了**（時間かかる）。**取得後 Claude が `build_sire_map.py`→sire_map.json→`experiment_sire_feature.py`(リークなしAUC実験)→効けば本番へ**。⚠32bit python=`AppData\Local\Programs\Python\Python312-32\python.exe`。COMを握るので**ライブ取得(KeibaDiffFetch)と同時に走らせない**（非開催の平日・取得プロセスが無い時に）。

---

## 🎯🔥 本人指示（2026-06-08・最優先・妥協&言い訳 禁止）出走の各馬を「一目で分かる美しい表」にする

本人がフジ競馬AI（@NeetFuji）の出走表スクショを見て強く指示：「**他のアプリはここまでやってる。うちの競馬アプリも、このレベル以上を妥協なしで作れ。今は10〜50点。100点を目指せ。**」

**お手本＝各馬ごとに1行**で、次がパッと並んで見える表：
- **能力値の横バー**（数値つき・バーで強弱が一目で分かる）
- **シグナルの色タグ**（本命=赤／対抗／注意=橙／抑え=黄 などの相対序列）
- **信頼度ランク**（A・B・C・D）
- **勝率／2着内率／3着内率**（％を各馬に表示）
- 人気・オッズも同じ行に

今の見た目は本人評価で「**醜い・見にくい**」。これを上記の美しさ・分かりやすさへ作り直すことを**最優先タスク**とする。同じ基準を姉妹アプリ「必殺一号艇」（各艇・各選手＋モーター評価版）にも適用し足並みをそろえる。

---

## ✅ 完了 (2026-06-08) — 各馬1行の評価表を刷新（上の最優先指示に対応・本番反映済み）

推定順位リスト（レース詳細モーダル）を、各馬1行の見やすい表に刷新。各馬に: **能力値バー**・**本命◎/対抗○/単穴▲/連下△の色シグナル**・**信頼度ランクA〜D**・**勝率/2着内/3着内**・人気/オッズ/期待値(EV)。

**2着内・3着内は「本物の実測値」**（本人指示「推定でなく本物の数字に」への対応）：過去 **3,689レース・50,742頭** の予想(`predictions/`の win_prob)と確定着順(`results/`の rank)を突き合わせ、予想勝率の帯ごとに「実際に何%が2着以内・3着以内に入ったか」を集計した実測キャリブレーション表を作成（`scripts/build-place-calibration.mjs` → `place_calibration.js` を index.html で読込）。＝「同じくらいの予想だった馬が、実際に何%来たか」の生の実績で、嘘ゼロ。表が未読込のときだけ Harville 推定にフォールバック。再集計はモデル更新後に同スクリプトを再実行するだけ。

実装は `app.js`（描画ブロック＋placeRatesFor＋harvilleフォールバック）と `styles.css`（signal-tag / ability-bar / rate-cell / conf-badge）。smoke 128/0 通過・main へ push 済み＝Vercel 自動デプロイで本番反映確認済み。姉妹アプリ「必殺一号艇」も同日に同基準で刷新済み。

**さらに「能力値」も実測ベースの絶対スコアに進化**（本人指示2回目）：旧「能力＝レース内1位馬を100にした相対値」を廃止し、`place_calibration.js` に各帯の絶対スコア `str`（＝勝率×0.6＋2着内×0.4 を史上最強帯=100に正規化）と帯中央 `pMid` を追加。`app.js` の `abilityFor()` が prob で str を線形補間 → **どのレースでも同じ意味の絶対実力**（弱いメンバーなら本命でも能力は低く出る＝正直）。信頼度A〜Dもこの絶対値基準。キャリブレーション未読込時のみ旧相対計算にフォールバック。

---

## ✅ 完了 (2026-06-09) — 取得スケジュールを「開催日カレンダー対応」に作り替え（本人指摘対応）

本人「**サーバーがメンテ中(rc=-504)の深夜に取りに行くな。いつレースやってるか調べてから取りに行け**」。調査で、今日(火)0:30・3:30 の取得が JRA-VAN メンテ時間帯に当たり **rc=-504 で5時間ぶら下がって失敗**、平日昼間も毎時「該当データなし」のムダ打ちと判明。JRA中央は土日(と3連休の月曜・祝日)のみ開催なのに、旧設定は**毎日16回**取りに行っていた。

直したこと:
- **開催日ゲートを `fetch_diff_hourly.ps1` に追加**: JV-Link を叩く前に本物のレースカレンダー(`data/jv_cache/races/*.json`)を見て、**土日 / 今日レースあり(祝日開催も自動で拾う) / 明日レースあり(前売り)** 以外の平日は **JV-Linkを叩かず即終了**。＝サーバーが閉まってる時間・レースが無い日に取りに行かない。火曜実走で「開催日ゲート: …JV-Linkを叩かず終了」を確認済み。
- **スケジュールを開催日に集中** (`scripts/register_diff_hourly.ps1` 改 + ライブ反映済み): `KeibaDiffFetch-*` を **毎日→週次トリガ**に。日中(09:00-17:30 毎時)=**土日月**、夕方の前売り(18:30/19:30/20:30)=**金土日月**。平日(火〜木)は毎日 **08:00** の `KeibaGapFill-0800` に任せる。**深夜 0030/0330/0630 は撤去**(メンテ門前払い・`register_overnight_gapfill.ps1` の作成箇所も削除)。
- **`KeibaGapFill` を 07:00→08:00 に移動 + 改名 (`-0700`→`-0800`)**: 07:00 は JRA-VAN メンテのシッポに時々ぶつかり aggregate が rc=-504 で失敗→`_status.json` が `aggregate_failed` の偽アラームになっていた(6/6・7・8 は07:00成功・6/9だけメンテが延びて失敗)。8時なら通常メンテ明け。※ pipeline は取得失敗でも既存データで予想/カード/push まで続行する設計なので、失敗時もサイトは前進する(平日は新レース無し=失う情報なし)。
- **他タスクは点検OK(変更不要)**: `catchup.ps1`(Navigator-Catchup 0800/1200) は中で「平日スキップ・土日かつ4h以上未取得の時だけ起動」と既に開催日対応済。`KeibaNavigator-HealthCheck`=土日09:00、`KeibaFridayHealthCheck`=金22:00、`KeibaSelfHeal`=土日昼+金夜 と、いずれも開催日に絞られている。

### 🔜 競艇式「発走◯分前だけ取りに行く」への進化 — 土台だけ完成 (2026-06-09)・本番化は土曜に検証して仕上げる

本人「競艇レベルまで賢く(各レースの発走30〜60分前だけ取りに行く)」。競艇(必殺一号艇)は既にこれを実装済(`/api/cron/tick` が各レース締切60分前〜30分後だけ重い処理・大村ナイター対応)。競馬にも同発想の部品 `jv_fetch.py cmd_watch`(発走60分前→5分/30分前→2分/10分前→30秒/+15分で終了)が**既に存在**するが、自動タスクは未使用(毎時取得のまま)。

- **判明した土台の欠落**: 競艇式の判定には各レースの**発走時刻**が要るが、`races/*.json` 3,732件すべてで `hassou_time` が欠損していた。原因＝`build_race_json.merge()` の出力辞書に `hassou_time` を入れ忘れ。RAレコード(`jvdata_struct.py:82` offset873)は元から持っているのに、`apply_tc`(発走時刻"変更"時だけ後付け)以外では一生入らなかった。
- **✅ 土台の修正(安全・取得の動きは不変)**: `build_race_json.merge()` の出力に `"hassou_time": ra.get("hassou_time")` を1行追加(2026-06-09)。ユニット確認済(ニセRAで merge→`hassou_time:'1540'` 出力)。→ **次回以降の取得で当日/翌日レースのファイルに発走時刻HHMMが自動で入る**(過去3,732件は確定済なので backfill 不要)。
- **🔜 土曜(6/13)に仕上げる本番化(未実施)**:
  1. 当日レースの `races/*.json` に `hassou_time` が実際に入っているか実データで確認(今日はJRA-VANメンテ中=`rc=-504` で取得不可・本人指摘の長時間メンテ)。
  2. `fetch_diff_hourly.ps1`(or 新スクリプト)に「各レースの発走時刻を見て active(発走45分前〜+15分)なレースがある時だけ RT/予想を回す。無ければ heartbeat だけで即終了」を実装(競艇 `tick` の anyActive ゲートの移植)。`hassou_time` が読めない時は従来の毎時動作にフォールバック(=絶対に今より悪くしない)。
  3. 毎時 KeibaDiffFetch-* を「5〜10分おきの軽いスマートtick(開催日のみ)」に置換。
  4. **土曜の実レースで「発走前に取れている」「直前オッズが新鮮」をログで見届けてから完了とする**(今日の教訓=未検証の自動取得を本番に流さない)。
- **ExecutionTimeLimit 30分**を全 `KeibaDiffFetch-*` に付与（今朝のような JVOpen 5時間ぶら下がりの再発防止）。
- 効果: 取得回数が **毎日16回(≈112/週・大半ムダ)→ 開催日中心の週36回前後**に。全部サービス時間内・レース日に集中。祝日開催はカレンダーを見るので取りこぼさない。
- ⚠ 注: ライブ登録は親子プロセスの権限差で `Register-ScheduledTask` が "already exists" になる癖があり、**先に全 `KeibaDiffFetch-*` を Unregister してから登録**する必要があった（次回再登録時も同様にクリーンにしてから流す）。
...
- **📚 8/12 過去10年ぶんのレースデータを取り込んだ（4,380 → 44,291 レース＝10.1倍）。毎朝の更新は1ミリも壊していない** — commit d02555f ほか。
  - **① 取れた**：`RACE option=3`（セットアップ）を 11:07〜14:07 で実行。**2,349ファイル中 2,282（97%）・11.0GB・212万レコード**。
    3時間の上限で止めたが、**2014-01 〜 2025-05-11 まで取れていて、手元データは 2025-05-17 から**＝**すき間ゼロ**
    （5/12〜5/16 は平日でJRA開催なし）。残りはもともと手元にある期間の重複なので損はしていない。
  - **② 生データを `aggregate_*` に置かなかった（ここが仕事の本体）**。置いたら次の3つが同時に起きる：
    ⓐ 翌朝の `build_all.py` が10年ぶんを展開（しかも**32bit Python**＝メモリ上限2GB）→ MemoryError
    ⓑ 差分ビルドは「dirty なレースを含む古いファイルも道連れで読む」作り。10年ファイルは**今日のレースも含む**ので**毎朝 11GB を読む**
    ⓒ races/ が 4,380→44,291 になると **features.json が 42MB → 452MB**。これは **git に載せて Vercel に配っている**ファイルで、
       **GitHub は 100MB 超を受け付けない → 毎朝の git push が落ちて本番が止まる**
    → **取得＝`setup_fetch.ps1 -OutDir history_raw` / 取り込み＝`import_history.py` → `data/jv_cache/history/races|results`** に分けた（血統の `kettou_raw/` と同じ考え方）。
  - **③ 壊していない証拠（全部 実測）**：`build_all` が読むファイル **9,536件・history は0件**／索引にも history **0件**／
    32bitの差分ビルド **1.8秒**（前と同じ）／**引数なしの `aggregate_features_v2` は 4,380レース・42,047,013バイトで いまの features.json と 差0件（完全一致）**／
    `_status.json` は取得前とハッシュ一致（`--outdir` 指定時は書かない作りにした）／history と本番で**同じ race は0件＝1件も上書きしていない**。
  - **④ 中身が本物である証拠**：日本ダービーと有馬記念を 2014/2017/2020/2022/2024 の10レース照合＝
    **ワンアンドオンリー／ジェンティルドンナ／レイデオロ／キタサンブラック／コントレイル／クロノジェネシス／ドウデュース／イクイノックス／ダノンデサイル／レガレイラ 全部一致**（等級・コース・頭数・単勝配当も）。
    3,000レース抜き取り＝**文字化け0・出走馬0頭のレース0・平均14.0頭・オッズ1.1〜939.3・人気1〜18**／結果2,000件中1,992件で1-2-3着がそろう。
    **grade_code（G1〜G3）も10年ぶん入っている**（本番の古いレースには入っていないので、そこは history 側が上）。
  - **⑤ 学習効果（リークなし・テストレースは A/B で同一の2,169R・5区間）**：
    **AUC 0.7710 → 0.7929（+0.0219）・5区間すべてで改善**。血統の +0.0010 の**20倍**。学習データは 29,467行 → 606,199行。
    🚨**でも回収率は 100% を超えない**＝13戦略中10戦略で上がったが **100%超えは A=0 → B=0**。
    いちばん件数の多い複勝（1,952点）は 80.7% → **79.7%（横ばい）**。いちばん良く見える「短距離の馬連」98.7% は**98点しかない＝運の範囲**。
    → **本番の予想ロジックには繋いでいない**（[[gambling-prediction-cannot-beat-takeout]] のとおり、当てる力が上がっても控除率20〜25%の壁は越えない）。
  - **⑥ 置き場所と容量**：`data/jv_cache/history/`（**694MB**・races 39,911 / results 39,270）＋`history_raw/raw_*.bin`（**11GB**）。
    **11GB の生データは消してよい**（もう JSON にしてある。消すと再取得に約3時間）。`features_10y.json` 452MB も実験用なので消してよい。
  - ⚠**教訓＝`build_all.py --full` を軽い気持ちで走らせない**。実測すると **4,380レース中 95レースの単勝オッズが書き換わった**
    （古い生データは容量のため削除ずみで、いま残っている生データからは当時の値を再現できない）。今回は控えから元に戻した。
  - 手順書＝`jv_bridge/SETUP_10YEAR_HOWTO.md`（実行ずみの内容に更新）。控え＝`控えとバックアップ\競馬_10年データ取込前_20260812\`。

- **🎯 8/12(夕) 5人がかりで「勝つ方法」を総当たり＝唯一の本物と、消すべき嘘を確定（本人「複数人で」「いっきに」「すべてよろしく」）** — commit 4c40183 / 2b352a1 / 204faff / 9fe371f ほか計27回。
  - **① 🎯 唯一 検査を全部通った買い方＝「オッズの安い1番人気の複勝」**（7,231通りを総当たり）。**単勝1.6倍未満で 95.83%（348回・9か月中9か月でベタ買いに勝つ）／3.0倍未満で 88.69%（2,961回）**。きれいに単調・切る場所を9通り動かしてもなだらか・**プラセボ（払戻シャッフル）では合格ゼロ/SPA p=1.000**（本物 p=0.0013）。**100年前から知られた favorite-longshot bias** そのもの＝掘り当てた偶然ではない。🚨**それでも100%未満**。85%の日に一番絞った形だけ **101.82%** になるが**95%の幅 [97.8, 105.9] が100%をまたぐ**＝アプリ自身の基準では不合格。画面には「おすすめ」ではなく **「買うなら これが一番マシ」**＋**「損がいちばん小さいのは、いまでも買わないこと」**と出す。天井の正体＝**複勝の払戻に100円の下限がある**（1.0〜1.3倍は的中93%でも平均配当104円・64%が元返し）＝天井 約96%。memory [[keiba-only-real-edge-favorite-place]]。
  - **② 📏「ものさし」を画面に常設**＝同じ645レースで「何も考えず1番人気の複勝を買う」と **84.9%**。比べる基準が無く「79.9%」が良いのか悪いのか分からなかったので必ず横に並べる。→ **14個の買い方のうち12個がこれに負けていた**。
  - **③ 🗑 ものさしに負ける買い方を画面から消した → 18個すべてが対象**になり「**いま おすすめできる買い方は ありません。見送りが正解です**」に。決まり＝①実成績がものさし以上 ②**150回以上**（アプリが既に使う基準にそろえる。新しい数字を作らない。実例＝87.5%だが**8回しか賭けていない**もの）③実績が無いものは出さない。⚠**黙って減らさない**（何個・何件を なぜ外したか必ず画面に書く）。**わざと壊して確認**＝ものさしを消すと4枚に戻る（安全側）・999%にすると0枚。
  - **④ ❌ 効かないと確定したもの（全部 本番未配線）**：**血統**（粗い使い方＝AUC+0.0010なのに**お金 −1.98pt**／別の使い方10通りも全滅。⚠**にせ血統でも +1.76pt 出る**＝本物の+1.81ptと見分けがつかない。⚠決定的な説明＝**血統が良い馬は実際に2.7倍走るが、オッズも同じだけ短い**＝もう値段に入っている）／**調教**6通り（大当たり3つ抜くと +3.43pt→**−0.01pt**・プラセボで−5.62〜+6.72pt動く・**データが濃い484Rでは −7.48pt**）／**券種どうしの裁定**（矛盾は本物で紙の上では31/432レースで作れるが、**15分ももたず・全レース適用で72.2%＝単勝ベタ買いより悪い**）。
  - **⑤ 🚨 実機の総点検で本物のバグ6件**：(a)**WIN5の折りたたみが ずっと真っ白**（開催日以外は `w5.strategies` が無いのに守りなしで読んで TypeError→「次回WIN5の予定」が一度も出ていなかった）(b)**下ナビ「履歴」が新規の人には死にボタン**（行き先が閉じた折りたたみの中で scrollIntoView が空振り。WIN5タブも同じ）(c)**馬番パレット18個中12個が読めない**（最悪2.28:1。勝負服の色は保ったまま実測係数で調整→**全部 4.5:1以上**・押しどころ28→44px）(d)🚨**「本番反映 2853日前(2018年)」という実在しない数字**＝**Vercelは配置ファイルの時刻を 1540000000000 に固定する**。偽物と分かる値は **null（分からない）** にし「この画面では計れません」に。**「分からない」と「壊れている」を区別する**(e)**存在しない「設定 → 自動化」への案内**（設定タブは 2026-06-23 に撤去ずみ）(f)記録ゼロの人に「達成バッジ」がまっしろ。
  - **🚨 この日の教訓＝検査自体が3回 嘘をついた**：①背景色を**文字列くらべ**（`"rgba(0, 0, 0, 0)"` の空白ちがい）で判定して全部を黒背景と誤判定 ②**子要素を持つ節を丸ごと飛ばして**わざと壊した色を見のがした ③背景が **linear-gradient** なのに `backgroundColor` しか見ず「白地に白文字 1.02:1」という嘘の数字を出した。→ **どの検査も「わざと壊して、ちゃんと落ちること」を先に確かめてから使う**。⚠**検査で本物のソースを書き換えない**（強制終了されると壊れた行が残る＝実際に残った）。**ブラウザ内に style を注入する**のが正解。
  - **⑥ 🕐 直前オッズの取得が 17:00 で切れていた**＝夏の新潟・中京は**2部制開催（暑さ対策）**で 1〜5R が 09:40〜11:50 →3時間20分あく→ 6〜12R が 15:00〜18:30。**1日6レースが丸ごと取れていなかった**。→ 18:50 まで延長。**netkeiba で2件照合**（レパードS 7R 15:35／関屋記念 第61回）して**うちのレース番号・時刻・等級が正しい**ことを確認（「重賞は11R」という思い込みの方が誤り）。点検 `scripts/near_post_verify.mjs` を **`keiba_learn_weekly.cjs`（月6:00）にぶら下げた**＝8/17朝に自動判定・止まりようがない。


- **🕐🧬 8/12(続) 直前オッズが17時で切れていた／JV-Linkの1年ごしの詰まりが解けた（本人「全部やりましょう」）** — commit b93ecd1。
  - **① 直前オッズの取得が 09:20〜17:00 で終わっていた（穴）**：「JRAは16:40には終わる」という思い込みで `KeibaNearPostOdds` を 7時間40分に設定していた。ところが**夏の新潟・中京は 2部制開催（暑さ対策）**で **1〜5R が 09:40〜11:50 → 3時間20分あく → 6〜12R が 15:00〜18:30**。＝**1日6レース（両場の10〜12R）が丸ごと取れていなかった**。→ **PT9H30M（18:50まで）に延長**＋`near_post_odds.ps1` に経緯を英語コメントで記録（ASCII専用を維持）。
  - **② 2部制が本物であることの裏取り**：自分のデータ359開催で「3時間以上あく」のは**28件だけ・新潟と中京だけ・2025年も2026年も同じ・いつも5Rのあと**。さらに **netkeiba で2件照合**＝`2026-08-09 新潟 7R 15:35 = レパードステークス(GⅢ)`／`2026-07-26 新潟 7R 15:45 = 第61回（関屋記念の回数と一致）`。→ **うちのレース番号・発走時刻・等級は正しい**（「重賞は11R」という私の思い込みの方が誤りだった）。
  - **③ 新設 `scripts/near_post_verify.mjs`**：本物の発走時刻と貯めたオッズの時刻を突き合わせ「発走20分前〜2分後に取れたレース」「直前5分を測れるレース（T-5〜T-0に2つ以上）」を数える。開催日なのに0件の日があった時だけデスクトップの箱に⚠（元気なら消す）。**いまの実測＝直前20分 44.8%・直前5分 0.0%**（＝2分おきは 8/15 が初回）。**新しいタスクを足さず `keiba_learn_weekly.cjs`（月6:00）にぶら下げた**＝8/17(月)朝に週末を自動判定・止まりようがない。
  - **④ 🚨 JV-Link の `rc=-501` の正体が判明（1年ちかく足踏みしていた原因）**：セットアップ窓の**既定が「スタートキット(CD/DVD-ROM)を持っている（推奨）」で、CD配布は2022年3月に終了**していた。「持っていない」を選べばよかっただけ。⚠**窓は2枚出る**（2枚目を押さないと JVOpen が無言で永久に止まる）／⚠**窓を隠して起動すると必ず固まる**。**`RACE option=3` で `rc=0`・2349ファイル＝10年ぶん開通**（約2GB DL開始を実機確認・`option=4` は 5.0.0 でも -501）。**本取得はまだしていない**＝生データを `aggregate_*` に置くと翌朝8時の `build_all.py` が10年ぶんの展開を始めて朝の更新が止まる恐れ。手順＝`jv_bridge/SETUP_10YEAR_HOWTO.md`。memory [[jvlink-setup-dialog-cd-default]]。
  - **⑤ 血統が繋がった（父 57.5% → 99.9%）**：`build_sire_map.py` に3つのバグ（一度も動いたことがない／`SK`に父の項目が未定義／新データを無視）→ 修正。**BLDN/DIFN は option=1 で窓が出ない＝全自動**。**私が独立に数え直して 60,199/60,235頭＝99.9% を確認**、リークなし検証も回し直して **AUC 0.7584 → 0.7594（+0.0010）・5期間中5期間で改善**を再現。⚠**父の産駒成績を出せるのは77.5%の馬**・**+0.001は小さい**ので**本番にはまだ繋いでいない**（回収率で確かめてから）。⚠繋ぐ時は **`sire_map.json` を `.gitignore` と `.vercelignore` の両方**に例外追加。


- **🏆💴 8/12 「常識を覆す」＝予想を当てずに控除率を下げる／G1判定が全滅していたのを根治／調教を繋ぐ（本人「全ていっきに」「常識を覆すことが大事」）** — commit e4907bd / fce42ab / 295b876。
  - **① 払戻率が上がる日をアプリの主役にした（唯一「確実に・誰でも」得をする差）**：ふつうの単勝・複勝は もどり80%（控除率20%）だが、**JRAウルトラプレミアムの日は 85%**（80%＋上乗せ5%）＝**引かれる分が 20% → 15%（4分の1が消える）**。予想の腕とまったく無関係。→ `lib/premium.js`（判定を1か所に集約）＋ **`/api/premium-days`（server.js と api/[...slug].js の両方）** ＋ `app.js renderPremiumDays` ＋ `#premium-mount`（結論カードより前）。**過去に85%の日が26日・80%上限の日が18日**（直近 2025-10-26）。**2026年の対象日はJRAがまだ公表していない**ので「該当なし」ではなく **「まだ公表されていません（今年は無いという意味ではありません）」** と出す。
    - 🚫**数字は1つも手書きしない**（＝6回やらかした同じ病気を繰り返さない）。「**回収率 94.1% 以上の買い方なら、この日だけは100%を超える**」は `100 ÷ (85/80) = 94.12` の計算結果。今の一番いい成績（86.45%・121回）は `/api/live-stats` から引き、**「この日でも 91.9%＝100%には届きません」と正直に出す**。最後に必ず「**勝てる日ではなく、負けが小さくなる日**」と書く。
    - 検品＝**`検品/premium-check.mjs`（実ブラウザ3機種・33項目・全合格）**。⚠**「わざと壊して落ちること」を先に確かめてから採用**（`--break=color/size/wide` の3通りとも検出）。
  - **② G1判定が 4,380レース全部で false だった（根治）**：`build_race_json.py` が `grade_code` を **"1"** と比べていたが、JRA-VANの実際の値は**英字**（**A=G1 B=G2 C=G3 D=重賞 E=特別 L=リステッド**）。生データで裏取りしてから直した。→ 再ビルド後 **G1 7件**で、**天皇賞(春)/NHKマイルC/ヴィクトリアマイル/オークス/日本ダービー/安田記念/宝塚記念** と実際の日程が完全一致。**2回目の裏取り**＝8/9の等級つき9レースも **レパードS(新潟ダ1800)とCBC賞(中京芝1200)が両方G3・残り7つは全部「特別」**で正解。⚠**grade_code を持つのは 2026-04-30 以降に取り直した生データだけ**（136日中30日）＝古い期間のG1は今後の再取得で埋まる。
  - **③ 調教（追い切り）を毎朝のパイプラインに繋いだ**：索引を作る `build_chokyo.py` が単発実行のままで放っておくと古くなる → `race_day_pipeline` に **step4.86**（`--skip-chokyo` で止められる）＋ **`/api/chokyo`**（本番200を実測）。⚠**出馬表が出るのは木・金**なので週末レースは金曜以降の再生成が要る。⚠**乗り役・併せ馬・脚色・外厩は JRA-VAN に項目が無い**（netkeibaのあれは独自取材）＝null で返す。
  - **④ 手書きの成績を撤去（6個目の同じ病気）**：WIN5の帯に「金の3連単 (期待 +129%)」「短距離・実力派 (+37%)」とあったが**実際は 0% と 49.5% でどちらも ✕止める**。→ 名前だけ残して数字を消した。あわせて発走後オッズ2件を隔離。**嘘チェック `検品/uso-check.mjs` が「おかしい 2件」→「0件」**に。
  - **⑤ 検品/ が公開されるところだった** → `.vercelignore` に追加。
  - **🚨 この日の教訓3つ**：①**「同じ検査」と書いてあるコメントを信じない**（`--break` で先に壊して、落ちることを確かめてから検査を採用する）②**検査で本物のソースを書き換えない**（強制終了されると壊れた行が残る＝実際に `.pd-math-key{color:#F2EDE3}` が残った。**ブラウザ内に style を注入する**方式が正解）③**別セッションが同じフォルダで動いている時は `git add -A` を使わない**（血統取り込みの一時ログを巻き込んだ）。
  - **🔬 並行調査の結論（本番は不変・memory [[parimutuel-arbitrage-is-a-mirage]]）**：**券種どうしの値段の矛盾は本物**（ワイド vs 3連複の総流しで**72.3%**が食い違う・最大の例は単勝4,673円 vs 3連単で作ると1,408円＝30%）。**紙の上では 432レース中31レースで「どう転んでも損しない」組合せが作れる**（100円単位・自分の購入によるオッズ低下を入れても残る）。**しかし成立しない**＝①最後の発走前に出た判定10件は**15分後に0件**②3連単プールは締切までの15分で**中央2.9倍**に育つ③本物の確定払戻で決済すると**18レース中5レースで元本割れ**。さらに**同じ作り方を全411レースでやると 72.2%**（単勝を全部買う76.3%より悪い）＝**「安く見える馬券」を選ぶこと自体がマイナスに働く**（安い理由は割安ではなく「まだ金が入っていない」から）。**副産物＝オッズからプールの総額が読める**（`総額 = 100 × 最高オッズの組の票数 × 最高オッズ`・残差ほぼゼロ）＝「あと何%来るか」が分かる道具。
  - **📌 観察（未解決・別件）**：**新潟・中京の 2026-08-08/09 は 5R(11:40) → 6R(15:00) に 3時間20分の空白**があり、6〜12Rが 15:00〜18:20 に並ぶ（札幌は 10:00〜16:00 で正常）。レース番号と発走時刻の対応自体は**東京11R 15:40＝東京優駿／阪神11R 15:40＝宝塚記念**で正しいことを確認ずみなので、**パースのバグではなく本物のデータ**の可能性が高い（2部制／薄暮開催？）。本人に確認するか、JRAの日程で裏取りする。


- **🚨🧹 8/11深夜〜8/12 「金払ってこれか」「1番人気ばかりしか買わない」「オッズ見てないよな」への全面対応（本人 強い叱責・6エージェント並行調査）** — 本人の指摘は全部当たっていた。実データで確認して直した。commit 42a98ce / 6d9b2fb / 333336d。
  - **① 画面に手書きの嘘の数字**：`app.js` のフォールバックに **BEST 126.8% / SAFE 106.3% がハードコード**されていた（実際は 79.9% / 77.7%）。削除し「成績データなし」と正直に出す。
  - **② 本当の成績をどこにも出していなかった**：`strategy_live_stats.json` は **14戦略すべて100%割れ・確定2,411件・合計 −151,620円**なのに、画面には検証(バックテスト)の良い数字だけ。→ **`/api/live-stats` を新設**し `#honest-record-mount` に必ず表示。⚠`.gitignore` と `.vercelignore` の**両方**に例外追加（片方だけだと本番未配信＝既知の事故）。
  - **③「1番人気ばかり」の正体**：実測 **4,380レース中 3,383レース(77.2%)で本命＝1番人気**。原因は `KEIBA_MODEL_BETA=0`（予想＝市場オッズそのまま）。一方 `predict_lightgbm.py` は **nopop(人気を見ないAI)** も計算しており **52.1%のレースで別の馬**を推していたが、**本番へ運ぶ処理が無かった**。→ `precompute_predictions.js` に `loadNopopPickFor()` 新設＋**API 2か所**(`api/[...slug].js` と `server.js`)を通して結論カードに表示。「どちらが当たる」とは言わず**見方を2つ並べるだけ**。※netkeibaも同じ設計思想（AIと人気のズレを別枠で見せる）と調査で確認。
  - **④ オッズを直前に見ていなかった**：`register_per_race_scheduler.ps1` は在るのに **`KeibaPerRace-*` の登録が0件**＝一度も動いていなかった。しかも**本物の `hassou_time` を使わず「09:50+25分×R」の決め打ち**。→ **`scripts/near_post_odds.ps1` を新設**（本物の発走時刻を読み T-20〜T+2 に入ったら RT オッズ取得→predict→precompute→push）。タスク **`KeibaNearPostOdds`**（土日月 09:20 から**2分おき7時間40分**・かくれ実行.vbs・窓ゼロ）。**境目を実証済**（25分前=無視／19分前=反応／発走1分後=反応／5分後=無視／90分前=無視）。テスト用に **`-TestNow "HH:mm"`** を用意（これが無いと窓の検査ができない＝実際に無効な検査を1回やってしまった）。
  - **⑤ 毎朝の取り込みがタイムアウトしていた**：8/11朝 `build_all` 600s・`aggregate_features` 300s で**両方タイムアウト→git push スキップ**（直近20日で6日発生）。そのせいで **8/8・8/9 の結果72レース分が未取り込み**＝検証サンプルが20%不足。→ 手動で取り込み（build_all は実測**約2分**で完走）、上限を **600→2400s / 300→900s** に。
  - **⑥ 古い集計 v1(リーク版)を既定オフ**：`aggregate_features.py`(v1) は**未来の結果が混ざる版**で v2 が後継なのに、毎朝 v1→v2 の順で**両方**走らせていた。v2 だけ失敗した日はリーク入り features.json でAIが学習する。→ 既定オフ（`--run-legacy-features` で復活）。**安全の根拠＝8/11 は v1 が落ちて v2 だけ動いたが正常に回った＝v2 単独で完結すると実証ずみ**。
  - **⑦ 見やすさ全面改修 `styles.polish.css` 新設**：今のテーマは明るい和紙なのに**暗テーマ時代の白文字が36か所置き忘れ**（実測コントラスト**1.1:1＝ほぼ読めない**）。→ 全部 4.5:1 以上（最小5.6:1）／**12px未満の文字58種→0**／**44px未満の押しどころ8種→0**。⚠**`sw.js` の `NETWORK_FIRST_PATHS` に `/styles.polish.css` を明示追加**（`/styles.css` は startsWith 判定で polish 版に当たらない＝入れ忘れると「直したのに古い見た目」）。
  - **⑧ うまみ買いの判定を厳しくした（いちばん大事な自己修正）**：`scripts/keiba_learn_weekly.cjs` を新設し タスク **`KeibaLearn-Weekly`**（毎週月6:00）に登録。最初は「150点以上・回収率100%以上・大当たり1回抜きでも100%以上」の3条件で **「馬連133.3% = ◯出してよい」**と画面に出したが、**360レース29,334通りの深い検証で これは見せてはいけない数字**と判明：**95%の幅が63〜247%で100%をまたぐ／オッズの26%が発走後で買える時刻に直すと121%→約100.7%／しきい値を過去成績で選び直す運用は実測94.2%で負け／EVが高いほど良いという坂になっていない**。→ **合格条件に「95%の幅の下限も100%超」を追加**し **◯出してよい／△まだ分からない(見送り)／✕止める** の3段階に。**現在「出してよい」は0件**、馬連は △（275点142.4%・大当たり抜き112.5%・幅67〜239%）。
  - **⑨ 戦略カードのラベル訂正**：検証値を「お金ベースの**本当の**回収率」と呼んでいた → 「検証（過去に当てはめた計算）」に。実運用の成績（例 49.5%・最大9連敗・✕止める）を必ず併記。
  - **⑩ `lib/overlay.js` のコメントの嘘を訂正**：「実測較正済なので**嘘のない期待値**」と書いてあったが、**較正を一度も読んでおらず、馬連の較正表そのものが存在しない**（`exotic_calibration.json` は wide と trio のみ）。生の Harville だと明記。
  - **⑪ `exotic_calibration.json` を最新化**：2026-06-28(3,747レース)のまま → **4,248レース**。指数 lam 0.85→**0.92** / tau 0.70→**0.62**。⚠**ベンターが香港で最尤推定した γ=0.81 / δ=0.65 とほぼ一致**＝この補正のやり方自体が正しいことの独立した裏付け。
  - **⑫ `/api/automation-status` がローカルだけ404**で実ブラウザ検査が毎回「JSエラー1件」の偽失敗を出していた → `server.js` から本番実装へ委譲（`res.status/.json` の薄いshim付き）。
  - **検証**：smoke 128/0／実ブラウザ(iPhone/iPad/PC) **33+48+14+6項目 全通過**（コントラスト・文字サイズ・押しどころ・横はみ出し・JSエラー・表示内容をすべて実測）。本番反映を sw v81 / `/api/live-stats` 200 / `/styles.polish.css` 200 で確認。
  - **📌 この夜の教訓**：①**自分が「合格」と出した判定こそ疑う**（3条件で通した馬連を、深い検証が「見せてはいけない」と否定した＝消そうとしていた嘘を自分でやっていた）②**検査が意味を持っているか自体を検査する**（23:43に「60分後のレース」を作ったら日付をまたいで無効な検査になっていた）③**新しいCSS/スクリプトは配信・タスクに繋ぐまで一度も効かない**（`.vercelignore`・`NETWORK_FIRST_PATHS`・タスク登録の3つとも今回踏んだ）。
  - **🔜 次の一手（効果が大きい順）**：**(1) 過去10年データの再取得**（8/6にJV-Link 5.0.0へ更新後、一度も試していない。最後の失敗は7/2 rc=-501。学習12倍＝企画書が「一番効く」とする本丸。**⚠GUIのクリックが要るので本人の操作が1回必要**）／**(2)「オッズが上がった馬は買わない」の検証**（`signals_validation.json` で money_out は3着内率14.79%＝全体−7.9pt・2,448頭。**見送りは控除率の壁を越える必要がない**のに、これまで「買う」方向しか検証されていない）／**(3) 直前5分のオッズ変化を特徴量にする**（Hanyu et al. 2025・JRA-VAN 89万件で係数−0.3386。⚠**15〜10分前だけだと符号が逆転(+0.3674)**＝必ず直前5分を切り出す。売上の47%が直前5分に入る）／**(4) 8/23ごろ late money の判定日**（`edge_watch.mjs` は30日スロットルで最後の本走が7/23。次は8/22〜23で、そのとき初めて有効20開催日を満たす）。

- **🚨🔗 8/9(土) 「オッズを開く」ボタンが403の画面に飛んでいたのを修理（本人がスクショ2枚＋強い怒り「ここから開いたのに開けない・本当に腹が立つ」）** — 100%こちらのミス。
  - **何が起きていたか**：`app.js` の `buildJraOddsUrl()` が **`https://www.jra.go.jp/keiba/program/2026/odds.html` を返していたが、そんなページは JRA に存在しない**（実測403 Forbidden）。＝**ボタンを押した人は毎回かならずエラー画面に飛んでいた**。当時のコメントが `// JRA 公式は別の URL 構造を使うのでトップへ` ＝**調べきれないまま「それっぽい住所」を置いて公開していた**のが原因。
  - **なぜ JRA 公式に直リンクできないか**：JRA のオッズは `JRADB/accessO.html` への **POST ＋ 使い捨ての合言葉** が要る仕組みで、レースごとの住所をリンクにできない（`/keiba/baken/` も403）。実在を確認できた公式ページは `https://www.jra.go.jp/`・`/keiba/`・`/keiba/thisweek/`・`/keiba/calendar/` の4つ。
  - **直し方**：raceId から **netkeiba のレース別オッズ**（JRAのオッズをそのまま出している）を組み立てる。`202608090101060100`(18桁) → `202601010601`(12桁) ＝ **西暦4桁 + 場コード + 回次 + 日次 + R**。**場コードは JRA-VAN と netkeiba で同じ番号**（01札幌〜10小倉）。ボタン名も正直に **「JRA 公式オッズを開く ↗」→「いまのオッズを見る ↗」**（公式トップへの入口はレース詳細の「JRA 公式へ ↗」に元からある＝200で健在）。raceIdが変な時は `/keiba/thisweek/`（実在確認ずみ）へ逃がす。
  - **検証**：①変な入力8件→全部 逃げ先 ②本番APIの**36レース全部**で 200 ＋ `<title>` のレース名・競馬場・日付の一致 ③**実ブラウザ（Playwright/本番URL）で画面のボタン14個の飛び先を読み出し**て全部200・JSエラー0。sw v78→v79。commit 938cde9 → 本番反映を実測確認（古い403の住所はコメント内だけ・コードには無し）。
  - **📌 教訓**：**外部リンクは「ありそうな住所」を書かない。必ず叩いて200と中身を確かめる。** リンクは押されるまで壊れていても静かに残り、本人が使いたい瞬間に裏切る。アプリを触ったら `grep -ohE 'https?://…'` で外部リンクを全部さらって一斉に叩く。memory [[external-links-must-be-opened-not-guessed]]。

- **🚨🔧 8/6(木) データ取りが2日間 止まっていたのを復旧＝JV-Link 4.9.0 → 5.0.0 に更新（本人「競馬のやつがダウンロードを求めてる」）** — 本人がEdgeに出ていたJRA-VANのダウンロードページを見て相談。調べたら**本物の故障だった**。
  - **何が起きていたか**：**JV-Link 5.0.0 が 2026-08-04 に公開**（32bit/64bit両方）→ **8/5・8/6 の `aggregate RACE` が `JVOpen rc=-1（該当データなし）` で 0 件**。8/4 までは毎日 `rc=0` で取れていた（ログ `logs/race_day_*.log` を並べて確認）。**古い版は配信を止められるが、エラーは1つも出ない**＝「今日はレースが無い日」とまったく同じ見た目になる。
  - **気づき方**＝JV-Link が勝手にブラウザを開いてインストーラーを落としており、`Downloads\JV-Link (1).exe`(8/5)・`(2).exe`(8/6) が**毎朝1つずつ増えていた**（中身は同じハッシュ＝入れるまでずっと聞いてくる）。
  - **やったこと**：①走っている `race_day_pipeline` が終わるのを待つ（08:10 完了を確認）②利用キーを控えに退避（`reg export "HKCU\Software\JV-Link"` → `控えとバックアップ\JV-Link設定の控え\HKCU_JV-Link_20260806.reg`）③署名を確認（`JRA SYSTEM SERVICE CO.,LTD.` の EV 証明書・Valid）④`Start-Process -Verb RunAs` で更新⑤検証＝利用キー `3UJC-46WW-7VV1-T7RX-4` 健在／`jv_fetch.py init` → `JVInit OK`／**同じ条件の aggregate で `rc=0 readcount=50` 5,187 レコード取得**⑥通し処理（race_day_pipeline）を1回まわして端から端まで確認。
  - **控え**＝`控えとバックアップ\JV-Link設定の控え\` に `JV-Link_4.9.0_もどす用.exe`（切り戻し用）・`JV-Link_5.0.0_20260806.exe`・利用キーの .reg。Downloads は掃除ずみ。
  - **🚨 見張りの穴も直した**＝`scripts/health_check.ps1` が `state=aggregate_no_data` を `[?]` として NG/WARN のどちらにも数えず、**データ0件でも「すべて緑」と報告していた**（＝読めていないのに合格を出す最悪の型）。→ `[WARN]` にして「JV-Link に新しい版が出ていないか」を案内する文面に。さらに点検タスク `KeibaNavigator-HealthCheck` は **土日のみ（DaysOfWeek=65）だった**ので **毎日9時（127）** に変更＝火曜に壊れても水曜の朝に気づける。
  - **📌 今後の教訓**：①**JV-Link の「ダウンロードして」は無視しない**（放置＝データが止まったまま週末を迎える）②`rc=-1` は「レースが無い日」と「版が古くて配信停止」を区別できない＝**過去数日のログと並べて初めて分かる**③32bit COM は `C:\Windows\SysWOW64\JVDTLAB\JVDTLab.dll`・利用キーは `HKCU\Software\JV-Link\ServiceKey`（更新しても消えない）。

- **👀 7/2(水) 見やすさ全面改修「この買い方がおすすめ」を主役に (本人「1点じゃなくてもいい・複勝だけでなく単勝/馬連/ワイド/3連単など“どの券種がお得か”を教えて・パッと見て買えばいいと分かるように」)** — 予想カード(`renderBuyCard`)を再構成。新関数 `buildRecommendedBuys(race,tier)` で **かたく(複勝)/お得(ワイド、無ければ馬連)/大きく(3連複・ultra/prime時のみ)** を、**実測の当たりやすさ%**(複勝=place較正／ワイド・3連複=`exotic.js`のディスカウントHarville較正)＋おすすめ金額(tier別)つきで大きく1枚表示。むずかしい数字(期待値・AI信頼度・Walk-forward・買い目の例・うまみ)は「くわしく見る ▶」の折りたたみへ集約。ごちゃつく「本命予想」帯を撤去・正直な注意書きを1行に短縮。styles.css末尾に `.reco-buy` 系。sw v73→v74。**検証**: node --check・smoke 128/0・Playwright(Chromium 390px)でフィクスチャ注入し `.reco-buy` 描画/レイアウト/JSエラー無しを確認→本番 push(commit 8de2bfa)、production app.js に `buildRecommendedBuys` 反映・sw v74 を実測確認。⚠正直: 「当たりやすさ」は実測の的中率であって儲け保証ではない(控除で平均マイナス)。7/2は非開催日のため本番は「休む日」表示、新カードは次の開催日(土日)に自動表示。
- **🛡 7/2(水) ③損を減らす規律を実装・公開 + late money 最新再計算** — ①**「今月の予算まもり」**カード新設(収支の下・`renderBudgetGuard`/`#budget-mount`)＝月予算をワンタップ設定(3千/1万/2万/3万/自分で)・今月使った/予算/残りをバー+3セル・8割で注意/超過で赤「今月はもう買わない」・1レース目安(予算3%)。localStorage `keiba_month_budget`。②**規律もう一歩**＝`monthBudgetState()`に予算状態集約＋`suggestStake(tier)`で**おすすめ金額を予算に連動**(予算がある時は1レース上限=予算3%で頭打ち。例:予算3000→おすすめ¥100)、**予算オーバー時**は「この買い方がおすすめ」を赤バナー+全券種「今月は見送り」に、記録(`submitAddBet`)は予算超過の新規支出を`confirm`で一度止める。sw v74→v76。検証: node --check・smoke 128/0・Playwright(390px)で予算連動¥100/オーバー時見送り+残り−¥500/JSエラー0 を確認→本番push(commit 8de2bfa系の後続)・production反映確認。③**late money 最新再計算**(`analyze_late_money.py`)＝有効6開催日(目標20)・回収率53〜75%・100%超え確率ほぼ0%(money out急騰のみ74.8%/7.9%だが少数)。**前回6/23の「76%/5.4%」はデータ増(4→6開催日)で「62%/0.1%」に下降＝少数のマグレと確認**。結論:まだ隙なし・20開催日(8月上旬ごろ)まで自動で待つのが正解。
- **🧑‍🔬 6/23(火) 「3人の専門家」体制で総点検＝研究/厳しく確かめる/使いやすさ (2026-06-23・本人指示「いい手法をリサーチする人・データが本当に正しいか確かめる人・世界一使いやすいか見る人、の3人を置いて作業を始めて。おかしいものは自分の判断で直して」)** — サブエージェント並列で実施。本番は壊さず、結論を出してから commit：
  - **使いやすさ監査→トップ5を修正・公開 (commit 3ac6fd3・sw v73)**：①iOSの白いお墓＝`.form-group select`/`.modal-body select` に `appearance:none`＋金▼SVG（NOIR側`!important`）②平日にいちばん見る `DEFAULT_STRAT_DEFS` の `short_label`/`label` から英語・記号(V-STACK/nopop/σ/avg/Walk-fwd/worst)を一掃し子ども日本語へ③押して無反応だった「設定」タブをボトムナビから撤去(5→4)④ヘッダーの "AUC" 表記を抹消⑤手動入力モーダル `92vh→92svh`＋本文スクロール。**EV/おすすめ/モデルは不変・表示と文言と崩れ対策のみ**。smoke 128/0。
  - **研究（勝つ人の手法）の結論**：世界一の馬券師Bill Benterの核心＝「自前予想と公開オッズを `exp(α·log f + β·市場暗示確率)` で合成して系統バイアスを消す」。日本はリベート構造が無い＝勝負どころは「市場を上回る予測精度」一点。詳細は会話ログ。
  - **厳しく確かめる人＝3つの提案を実データでleak-free検証**：①Benter合成→**幻**（既存value_ev_bets.jsonでα・βを過去だけ最尤推定すると毎期間 **α=0／LL改善ゼロ**＝今のモデルは市場に1ビットも勝てていない。AUC モデル単独0.75/市場単独0.84。2次元補正の「79%が上限」と同じ壁）。②late money(直前オッズ変化)→**唯一の本命だが当時の+9.3ptは結果リーク込み**。③前走上がり2位×先行→**既にpace実験(AUC-0.0011)で否定済みの言い換え**。＝**手元データは飽和**。本筋は新データ取得と確定。
  - **本筋「血統・調教」をステップ1〜2まで実施→いったん行き止まり**：①取得可否＝血統(BLDN:HN/SK/BT)・調教(SLOP=HC坂路/WOOD=WC・haron_4)とも取得OK・ketto_numリンク良好を実証。②本取得＋leak-freeオフライン実験(experiment_pace方式)＝**調教3特徴(直近4F/偏差/経過日)はAUC上積みゼロ(-0.0001〜-0.0006)＝飽和の言い換えで見送り**。**血統は検証不能**＝現役馬の父データは UM(馬マスタ) にあるが `UMA` dataspec が **rc=-111(非対応)** で取得不可、BLDNのHN/SKは仔馬マスタで現役馬と58頭しか重ならない。→**現契約・dataspec構成では血統の本番投入に必要な父データを集められない**（将来UMAのrc=-111が解ける手段が出たら血統だけ再開）。本番無改変・取得binは全削除（build_allが毎時拾う事故を回避）。
  - **late money(賢いお金の流れ)を「お金ベース回収率＋ブートストラップ」でleak-free判定→保留**：締切後の確定オッズ混入を除くと有効は実質4開催日(6/13・14・20・21)・137レース。money_inの3着内率エッジ(+9〜13pt)はリーク除去後も生存するが、来るのは「人気で安い馬」＝**回収率は控除率(20-25%)を超えず**(単勝CIすべて100%をまたぐ/複勝は明確に負け)。「やや下落-15〜-5%単勝116%」「下落×中オッズ7-15倍144%」は超少数＋日替わり47〜192%でブレ＝幻の99.5%と同型。**判定不能・最低20〜25開催日(3〜4か月)の発走前オッズ蓄積が必要**（archive_signalsは既に正しく蓄積中）。
  - **🔧 実在バグ修正・公開 (commit 294a45b)**：上の検証で `race_facts.py signal_odds_moves` の delta が**締切後の確定オッズ(SE win_odds)をlastに使う結果リーク**(採点216レース中186=86%)と判明→**発走2分前より後/確定オッズ一致の最終スナップショットを除外**し発走前オッズのみで変化率算出に修正。効果＝money_in 3着内率 0.3242(ベース+11pt)→**0.2649(ベース付近)**＝甘い優位性が幻と確定。これで今後の蓄積を正しい物差しで測れる。**delta作り方のみ変更・EV/おすすめ/事実メモ表示は不変**。smoke 128/0・pytest 245 passed・build_race_card exit0。
  - **総括（正直）**：今日この瞬間に予想を強くする一手は見つからず。だが①ニセ強化3つ(Benter/血統調教/上がり交差)を本番投入前に全部はじいた②late money計測のリークを直し将来の判定を有効化、が成果。**真の上積みは「より長い発走前オッズ蓄積(3-4か月)」と「血統の父データ取得手段」待ち**。`UMA` rc=-111 の回避、または別データ源が次の鍵。
  - **🛡️ 恒久対策＝月1自動「強化チェック見張り役」を常駐化 (本人「真剣にやったのに“何も無し”で終わる虚しい流れを二度と繰り返したくない／何とかして」)**：手作業の大調査をやめ、裏で自動監視に切替。`C:\Users\shoug\自動実行ツール\edge_watch.mjs`（毎日04:50起動・自前スロットルで中身は30日に1回・`かくれ実行.vbs`経由で窓ゼロ・タスク`EdgeWatch-Monthly`・StartWhenAvailable）。中身＝leak-freeで①2次元補正EV(`analyze_calibrated_ev.py`)②late money回収率+ブートストラップ(`jv_bridge/analyze_late_money.py`新規・計測専用・本番不接触)を回し、**「ブートストラップ下限5%が100%超 かつ 有効20開催日以上」を満たした時だけ**両デスクトップに `⚠競馬に強化のチャンスが来たかも.txt` を置く（無い時は消す＝静かに待つ・Kelly/iPad点検と同流儀）。現状は必ず `はじめアプリ\競馬の強化チェック.txt` に正直な数字を上書き（試運転＝2次元補正EV最良76%/100%超え確率0%・late money 76%/有効4開催日/5.4%＝まだ無し）。試運転 LastTaskResult=0x0。**これで「どう？→ダメでした」の虚しい報告会を廃止し、本物が出た時だけ知らせる方式に**。本番EV/おすすめ/モデルは不接触。commit 097dd85(計測スクリプトのみ・edge_watch本体とタスクは自動実行ツール側＝repo外)。

- **🚨🩹 6/13(土) 本番「オッズ全滅事故」の根治 (2026-06-13 13:30・本人が強い不満)** — 本人「オッズが全滅・予想ができない」。調査の結果オッズ自体は生成されていた(未来17レース全部にオッズ有り)が、**2つの本物のバグ**で本人の画面が「全滅」に見えていた:
  - **バグ1 (毎時取得の二重起動・thundering herd)**: パソコンが午前中止まっていて 09:00〜12:30 の `KeibaDiffFetch-*` が溜まり、復帰した **13:18 に5本同時起動**。5本が同じ JV-Link と features.json を奪い合い `aggregate_features_v2` が5重並走 → どれも完走できずオッズ更新が朝8:03から停止 → アプリの安全バナー「予想は当てになりません」が出っぱなし=本人の言う「全滅」。**根治**: `fetch_diff_hourly.ps1` に**単一起動ロック**を追加(`data/jv_cache/fetch_diff.lock`・保持PIDが死亡/25分超で自動失効=自己修復)。これで何本タスクが重なっても本処理は1本だけ。(commit 0264332)
  - **バグ2 (当日カードが翌日へ飛ぶ)**: `build_race_card._pick_latest_real_date` が「オッズ+予想が揃った**最大日付**」を選ぶため、土曜午後に**日曜(翌日)の前売りオッズが出始める**と当日6/13を飛び越え 6/14(絶好機0件)を表示=おすすめが消える。**根治**: 「今日にまだ結果が出ていないレースがある間は必ず今日を表示・今日が全部終わった夜だけ翌日前売りへ」に変更(JST基準・commit e6d7615)。
  - **復旧確認**: 重なった取得を停止→鍵入りで1本だけ流し直し→**本番 keiba-navigator.vercel.app が 当日6/13・絶好機7+勝負4=11件をオッズ付きで表示・予想は数分前=安全バナー消灯**を実機API確認済み。
  - **⚠ 教訓**: ①毎時起動の自動タスクには必ず単一起動ロックを付ける(溜まって一斉起動する) ②「最大日付を選ぶ」系ロジックは前売りが前日に出ると当日を横取りする→必ず「今日」を優先する基準にする。

- **🔬 6/13 本人「競馬で勝った有名な人たちのシステムを学んで取り込め」→ 調査+2次元補正+信頼区間まで実施 (2026-06-13)** —
  - **調査(背景エージェント)で判明した本物のエッジ**: 大阪/北海道の外れ馬券裁判の人たち(回収率104〜108%)も「予想の天才」ではなく、**favorite-longshot bias(=みんな大穴を買いすぎ→大穴は割高・本命は割安)を突き、割安な馬を機械的に大量購入して薄利を積んだ**だけ。大数の法則は分散を下げるだけで期待値は上げない(エッジが先)。ポイント還元で勝つのは神話。自動投票はJRA規約グレー=作らない。詳細な情報源は会話ログ参照。
  - **「平均の罠」対策をアプリに実装(本番反映済)**: 戦略の回収率表示を「お金ベースの本当の回収率(総払戻÷総投資)」主役に変更し、期間別の平均がそれより目立って高い戦略には「⚠この平均は大当たりで釣り上がっている・本当は◯%」警告+判定ラベルを表示。app.js 2箇所(1330/1468ブロック)・styles.css(trap-warn/trust-note/is-loss)・sw v72。実ブラウザで表示確認済。
  - **2次元補正+信頼区間 検証ツール新設** `jv_bridge/analyze_calibrated_ev.py`: leak-free な per-bet キャッシュ(value_ev_bets.json)から、(予想勝率帯×オッズ帯)→実勝率 の2次元補正表を**expanding window(未来を覗かない)**で作り、補正EVで単複購入+レース単位ブートストラップ1000回で回収率の信頼区間を出す。**結果(清浄データ・お金ベース)**: 生EV 65% → 2次元補正+オッズ3-15倍で**79%**まで改善(=補正は本物に効く・基準の人気1番76%も上回る)。**ただし依然100%未満**。最良戦略でもブートストラップの100%超え確率6.8%=運の範囲で**確定的な勝ち戦略は無し**(memory [[keiba-no-positive-ev-current-data]] を組み合わせ馬券+2次元補正でも再確認)。
  - **配線を試みて"罠"を検知→安全に保留 (2026-06-13・本人は「今すぐ出せ」だったが出さない判断)**: `build_2d_calibration.py` で本番用補正表を作り `honest_ev.py` に配線したが、テストで**人気考慮(primary)モデルの勝率で補正すると、その勝率は既にオッズを織り込んでいる(循環)ため本命のEVが過大評価**になる罠を検知(例: 勝率0.45×2.0倍が正直EV1.36と出る=「絶好機」乱発の悪化)。leak-free検証では補正版も79%で負けと出ているのに個別EVだけ良く見える=過去の「幻の99.5%」と同型。→ **本番には出さず、`ENABLE_CAL2D=1`(環境変数)を立てた時だけ有効になる二重安全装置を入れて既定OFFでコミット(commit 5d9b2ec)。補正表は `.held` に退避し本番は従来の cool_factor のまま**=今日のおすすめは不変。
  - **🔜 正しい完成手順(次回・非開催日に)**: ①`build_2d_calibration.py` を **odds-blind(nopop)勝率**ベースに直す(predictions の nopop 勝率。primaryはオッズ循環で不可)。②`analyze_calibrated_ev.py`(leak-free)で**過大評価の幻が無い**ことを再確認(個別EVだけでなく実回収率・信頼区間で判定)。③問題なければ `ENABLE_CAL2D=1` を pipeline に設定して有効化。**当日のEV/おすすめを変える変更は開催日を避ける**(本人混乱防止)。④`calibration_2d.json` の定期再生成を `CalibrationRefresh-Weekly` に追加。
  - **🧪 地力アップの実験(option2)結果と次の段取り (2026-06-13)**: 「展開(逃げ・先行馬の数)」特徴を**オフラインでリークなし検証→AUC 0.7594→0.7583で効果なし**と確定(`jv_bridge/experiment_pace_feature.py`・本番未配線)。条件別成績(course/distance/surface/goingWinRate)は既にfeatures.jsonにあり=既存モデルは手元データでほぼ飽和。**真の上積みには新データ取得が必要**。本人指示で**次の非開催日に「血統」と「調教(追い切り)」の取得→特徴量化を腰を据えてやる**ことが確定。
  - **📋 次回(非開催日)の血統/調教プレイブック ※本人は「血統データを入れて」等と言うだけでよい**:
    - **データは取得可能と確認済**: 血統=`HN`(繁殖馬・父母 hansyoku_f/m_num)/`SK`(産駒)/`BT`(系統名 keito_name)、ketto_num で各馬とリンク。調教=`HC`レコード(haron_4=4ハロンタイム, lap_4 等)。struct(`jvdata_struct.py`)に定義済。
    - **手順**: ①`jv_fetch.py aggregate` に血統系dataspec(例 BLOD/血統一括)と調教dataspec(例 WOOD/調教)を追加取得→parse→キャッシュ保存。②**リークなしの特徴量**を作る: 血統=「種牡馬(父)の産駒の、その馬場/距離での1着率」(当該レース以前の集計のみ)、馬齢若い/出走少ない馬ほど効くはず。調教=直近追い切りのタイム偏差。③**必ず先に `experiment_pace_feature.py` と同じオフライン実験台でAUC上積みを確認**(効かなければ本番に入れない)。④効けば aggregate_features_v2.py + train_lightgbm の FEATURE_NAMES に追加→再学習→walk_forward_true_auc で再確認→**非開催日にpush**。⑤エンジン同時起動禁止・窓ゼロ等の既存ルール順守。
    - ⚠ JV-Linkは32bit(`py -3.12-32`)・学習/分析は64bit(`Python312-64`)。lightgbm/sklearnは64bitのみ。

- **🚨 6/6-6/7 土日本番の大失敗と根治 (2026-06-07 夜・本人「なんでできなかったのか説明しろ」)** — 土日二日間、当日オッズが一度もアプリに流れず、全レース一覧は5/31のまま・おすすめ0件だった。**タスクは全部「成功(0x0)」表示で、ログ上も正常に見えていた**のが最悪だった。原因は2つの本物のバグ:
  - **バグ1 (レースID桁違い)**: Wave36 で入れた RTオッズ取得が、races/ のファイル名(フロント互換で末尾00を足した**18桁**)をそのまま JVRTOpen に渡していた。JVRTOpen が受けるのは**16桁**。全RT取得が rc=-114 で全滅。しかも -114 は「未発売」と同じコードなので、金曜夜の事前テスト(未発売=失敗が正解の時間帯)では**バグと正常拒否の区別がつかなかった**。→ jv_fetch.py cmd_rt で18桁を自動で16桁に正規化 (commit c8a41c8)
  - **バグ2 (オッズ未配線)**: そもそも build_race_json.merge は `o1["win_odds_by_horse"]` を読むのに、**それを作る処理がコードのどこにも存在しなかった**。つまり仮にRT取得が成功していてもオッズは画面に流れなかった (過去レースは SE の確定オッズで偶然見えていたため誰も気づかず)。→ parse.parse_raw_file で O1 の繰り返し領域をパースして win_odds_by_horse / popularity_by_horse を添付。発走前の人気も O1 で補完
  - **黙る設計の廃止**: fetch_diff_hourly.ps1 は RT 失敗を Out-Null で捨てていた → rc 内訳を必ずログ + 発売中時間帯(9-16時)に当日レースありで0件成功なら **[ALERT]** を出すように
  - **実走証明 (当日夜)**: 24/24 RT取得成功 → 今日の阪神12Rに本物オッズ(3.2倍/人気1番)が入る → race_card 日付20260607・絶好機3/勝負2/様子見8/見送り11。smoke 128/0・pytest 245 passed
  - **検証エージェントの教訓**: 土曜8:10の自動チェックは異常を正しく検知して「今すぐは使えません」と書いていたが、**Gmail下書きのままで本人に届かず**、原因推測も外れていた(PCスリープ説)。報告は下書きでなく届く手段にすること・「rc=-114 だから正常」と機械的に流さないこと
  - **⚠ 来週末への正直な注意**: 今回の修正は「レース当日の発売中時間帯」での毎時実走はまだ証明できていない(今日は閉場後の実走証明)。来週土曜 9:30 の毎時取得ログで `RT odds fetch done (24/24 ok)` と `git push` を確認するまで完了と言わない
  - **バグ3 (馬データの無限重複・別セッションで同夜に修正)**: build_all の group_by_race が毎時届く同じレースの SE (出走馬) レコードを**全部追加し続けて**いた → 1レース16頭のはずが「同じ馬5重複+枠順確定前の空シェル(馬番0)32件=112頭」の壊れた races/*.json が量産され、当日の予想が全レース「判断不可」・本番の本命表示0件になっていた (既知の「同一馬3重複」バグの正体)。さらに**汚れたデータで学習したモデルは AUC 0.82→0.74 まで劣化**していた(本人の「精度が悪い」体感の原因)。→ build_all.py に dedup_se_list() を新設(馬番1+があれば空シェルを捨て、同馬番は「確定着順あり>情報量>新しい方」で1件に)。再構築後 16頭・モデル再学習で AUC 0.8163 に回復
  - **バグ4 (本番の WIN5 が一度も動いていなかった・同夜修正)**: 本番 (Vercel) には races/*.json が無い (git 管理外) ため /api/win5 の readAllRaces() が常に空 → **公開以来ずっと 503**。画面確認は CDP の試験データ差し込みで行われていたため気づかず。→ precompute_predictions.js が当日+翌日の完全レースデータを `today_races.json` (git 管理・約150KB・平日は空) に書き出し、lib/jv_cache.readAllRaces がフォールバックで読む。**本番 /api/win5 が 200 で本物データを返すことを実機確認済** (commit 3cdb804)
  - **同夜の追加防御 (本人「今直せるものを全部直せ」指示)**:
    - **アプリに緊急警告バナーを実装** (sw.js v71): 開催日の発売中(9:00-16:59)に「今日のレースに本物のオッズが無い」or「予想の計算が2.5時間以上前」なら、画面最上部に赤い「自動更新が止まっています — この画面の予想は当てになりません」を自動表示 (`computeStaleAlert`/`renderStaleAlert` in app.js・`#stale-alert-mount`)。`?staleTest=1` で強制表示して実機スクショで動作確認済み。**下書きメールと違い、アプリを開けば必ず目に入る**のが本人への確実な警報経路
    - **金曜夜の前売り警報**: fetch_diff_hourly.ps1 に「19時以降に翌日レースの RT オッズが0件なら [ALERT]」を追加 (前売りは前日夕方開始のため、土曜の故障を金曜夜のうちに検知できる)
    - **遠隔の自動点検2本を予約** (claude.ai/code/routines・PCにもセッションにも依存しない): ①金曜 6/12 20:50 JST 前売りチェック (trig_01CRuxV6cs64u1QzYPQcezpV) ②土曜 6/13 9:45 JST 本番チェック (trig_01EKrEetgruJxVLtNAdiucgU)。**報告は Gmail 下書きに加えて Google カレンダーのイベント(通知リマインダー付き)を作成** = スマホに必ず通知が出る。「下書きが読まれず異常が伝わらなかった」事故の再発防止
    - **金曜 6/12 夜の前売りオッズ取得が修正後初の本番証拠になる** (土曜を待たずに分かる)
  - **🚑 自動救急隊 KeibaSelfHeal を新設 (2026-06-08 未明・本人「鳴らすだけでなく、すぐ自動で直せるようにしとけ」指示)**:
    - `scripts/self_heal.ps1` — 開催日の発売中 (土日 9:48/11:48/13:48/15:48) + 金曜前売り (20:48) に健康診断。異常 (当日オッズが100分停止 / predictions 2.5h 古い / race_card 日付ずれ) なら:
      **1段目**: 詰まった jv_fetch プロセスを掃除 → fetch_diff_hourly をその場で再実行 → 再診断
      **2段目**: それでも直らなければ **Claude を自動起動** (`claude -p ... --dangerously-skip-permissions`・最大25分・2時間クールダウンで多重起動防止)。指示書は `scripts/self_heal_prompt.md` (誤報確認→診断→最小修正→smoke/pytest→実走確認→push→報告 `logs/self_heal_claude_report.txt`)
    - 前売りモード (金曜夜) は1段目まで (前売り開始が遅い日の誤報を避ける)
    - タスクは「かくれ実行.vbs」経由 (窓ゼロ起動ルール準拠)・WakeToRun・実走で 0x0 確認済み
    - **検証済み**: ①通常実行=対象外で即終了 ②-Force で 異常検知→1段目→再診断→2段目引き継ぎ の流れ ③Claude救急隊の出動訓練も実走 (誤報を正しく見分けて引き上げた)
    - これで「異常 → アプリが赤く叫ぶ (本人保護) + 救急隊が自動修理 (復旧) + 遠隔点検がスマホに通知 (報告)」の3層が完成

- **🔬 きれいなデータで「ズルなし検証」を全部計り直し (2026-06-08 未明・本人「2=精度アップの研究を進める」選択)** — 重複バグはかなり前から存在＝**過去の全検証が汚れたデータの上に立っていた**ため、掃除後データで再計測。commit c9a7d1e push済:
  - **オッズ充足率 92.7% → 96.9%** (O1配線修正で過去レースの欠損も埋まった=検証の土台が改善)
  - **旧・有望株「単勝・1番人気でEV1.15以上=99.5%」は汚れたデータの幻と確定** (清浄データでは53.6%・2/6期間)。Wave36の実験室首位は取り消し
  - **100%超え かつ 80件以上のポケットは依然ゼロ** = 「市場効率的・確実に勝てる買い方は無い」の結論は清浄データでも不変 (memory keiba-no-positive-ev-current-data は正しいまま)
  - 実験室の正直表示を更新: 最良 81.3% (複勝・補正勝率0.35以上) / ものさし(1番人気ベタ買い)83.6%。rankings / recommendations も再生成 (best戦略 100.9%/57件は維持)
  - **次の本物の伸びしろ**: ①毎時RTオッズが今週末から初めて貯まる→「賢いお金の動き」(オッズ急変)の検証が数週間後に可能になる ②10年データ(option=4)は rc=-501 で引き続き不可 (2026-06-05・本人指示「効率が悪いなら直す・無駄じゃないなら触るな」) → 結論: 無駄ではない・触らない**: ログ実測で検証。①空振り時(新データなし)は**1回約14秒で即終了**(JVOpen rc=-1=データなし正常終了→RT試行→終了) ②新データがある時だけ重い本処理(build+予想+特徴量+学習+push=約4分。今日6/5は11:30の1回だけ) ③モデル再学習は6時間以内ならスキップ済み ④夜中の回(00:30/03:30/06:30)も各14秒=蓄積系の夜間配信の取りこぼし防止として妥当。唯一の微小な無駄=前売り開始前の明日レースへのRT単複オッズ試行24件×約13秒(金曜昼など)だが、前売り開始時刻は日により変わるためスキップ条件を入れると**取り損ねリスク>節約(1日2〜3分のCPU)**で見送り。

- **⚡ サーバーの地域を東京(hnd1)に固定 (2026-06-05・本人指示「いつも遅い・速くして」)** — commit a7633f4 push済・切替確認済: 計測で API が iad1(アメリカ東海岸)で動いていた(`X-Vercel-Id: hnd1::iad1`)。`vercel.json` に `"regions": ["hnd1"]` を追加。**実測: /api/races 1.4秒→0.29秒・/api/status 0.25秒・/api/race-card 0.27秒**。トップの体感(Playwright)は元から0.45〜0.9秒と良好で、ページ自体は静的配信＝主に開催日のAPI応答が速くなる。⚠️ 別件メモ: ローカル版(server.js)の既定ポート8765は現在「将棋ツッコミ動画工房」が常用しており**取り合いになる**。ローカル版を使う時はポート変更を検討（本人がローカル版を使うかは未確認）。

- **🏇 NOIR II — RACE DAY: 開催日画面 (土曜本番) も同じ品格に磨き上げ (2026-06-05・本人が選択肢「2」を選択)** — commit 194937f push済・本番反映確認済:
  - **開催がない日に開催日画面を再現する検証手法を確立**: CDP の Fetch インターセプトで /api/races と /api/win5 に本物構造の試験データを差し込み、結論カード(ULTRA/PRIME)・見送りリスト・全レース行・WIN5(ok状態)を実機撮影して磨いた。道具は `%TEMP%\keiba_design\shot_raceday.js` + fixture_races.json + fixture_win5.json (テンポラリ・消えたら作り直す)
  - **直した事故2件**: ①結論カードの場名「東京」が縦に1文字ずつ折れる ②本命3頭シルク行・買い目の馬名が1文字ずつ折れる → nowrap+省略(…)+セリフ書体の固定サイズで根治
  - **磨いた箇所**: TODAY'S BEST PICK 金塗り帯→静かな眉+ティアピル三重重複の解消 / 買い目役割タグ(緑青紫)を金の輪郭1声部+金額は右に縦積み / 円グラフ・ハート(赤緑橙)を金へ+円グラフ中心を漆黒に / 全レース行のティア塗り分け→罫線+時刻・期待値をセリフに / WIN5の紫を全廃(戦略カード・予算ボタン・第N戦=イタリック金・確率バー金・次回予定ボックスのインライン紫も app.js から除去) / AI根拠ボックス(検証+思考プロセス)を漆黒+金の番号輪に
  - **検証**: smoke 128/0 / 360〜1280px 横はみ出しゼロ / sw.js v70 / 本番反映を確認。実装は styles.css 末尾「NOIR II — RACE DAY」ブロック(約400行)

- **🔍 Wave37-QA — 土曜本番前の独立最終点検もう一周 (2026-06-05 未明・本人「2=最終点検をもう一周」指示。NOIR II / Wave36-C 完了後の状態に対する別セッションからの全数チェック)** — **全項目合格・修正ゼロ**:
  - テスト: smoke 128/0 / pytest 245 passed・6 skipped / app.js・sw.js 構文OK
  - 実機 (CDP スマホエミュ390px + PC 1280px): 横はみ出し0・JSエラー0・コンソール警告0。NOIR II (MAISON) デザイン正常描画 (紋章マストヘッド・REST DAYヒーロー・戦略カード金/白/灰)
  - Wave37 機能が NOIR II 後も全部生存: ↻更新ボタン / 引っ張って更新バー / スナップショット即時復元+保存 / 折りたたみ記憶 (9個) / 先行フェッチ消費 / 効果音トグル (ON/OFF文字に更新済)
  - 本番: sw v69 配信中 / NOIR II 反映済 / 先行フェッチ入りHTML / API CDNキャッシュ HIT (recommendations 46ms)
  - 自動化: Keiba系タスク21個 全部 State=Ready・LastResult=0x0・次回実行予約済 (土曜は GapFill-0700 → 09:00〜20:30毎時)。fetch_diff_hourly.ps1 に predict→race_card→precompute→push チェーン実装を実コードで確認。Wave36-C の実走証明 (0030 強制実行 0x0) はタスク履歴 (LastRun 6/5 01:50) と一致
  - 注意した点: 検証時のローカルサーバー初回起動直後は /api/races の初回読込が10秒超かかり「機能が壊れている」ように見える (2回目から正常)。検証は2回走らせて判断する

- **🏛️ NOIR II — MAISON: デザインを30点→100点へ全面磨き上げ (2026-06-04 深夜・本人「デザインは30点・100点を目指して全力で・他セッションが終わってから動け」指示)** — Wave37 (操作性) と総点検の2セッション終了を見張りで待ってから着手。commit f73b234 push済・本番反映確認済:
  - **30点の原因を実機撮影で特定**: ①アプリ名がスマホで2行に折れる ②黒×金テーマに大量のカラフル絵文字(🏠🎰🏆等)が混ざり安っぽい ③戦略カードに橙/赤/緑の枠が混色 ④折りたたみが同じ箱の羅列で単調 ⑤全部が箱の中の箱で高級ブランドの余白がない
  - **マストヘッド刷新** (index.html): 蹄鉄の紋章SVG + 飾り罫線 + Cormorant Garamond セリフ書体の銘「競馬ナビゲーター」(1行固定) + タグライン「期待値で選ぶ、買わないAI」+ 日付/本日R/勝負R/AI精度のメタ行。ids (brand-day/metric-races等) は全部維持=app.js 無改修で動く
  - **折りたたみ9つを「01-09 番号付き目次」に再構成**: CSSカウンターでイタリック金の番号を自動採番。箱をやめ罫線(hairline)区切り+セリフ見出し+細い輪のシェブロン(開くと金で90°回転)。h-icon絵文字スパンはindex.htmlから削除
  - **絵文字を全UI排除**: 買い目の印は競馬の伝統記号「◎○▲△☆」へ。アイコン類はCSSの font-size:0 + ::before「◆」(金) トリックで置換 (rc-icon/ps-icon/ach-icon等)。トースト印は ✓/!/×/i。効果音トグルは ON/OFF 文字。紙吹雪もシャンパンゴールド系に
  - **色を金/白/灰の3声部に統一**: strat-trust-card / rec-strat-card / ml-strat / ml-recommended の橙/赤/緑の枠と塗りを一掃。数字は Cormorant の細く大きな金 (37px)。「危険」系は静かな弱バラ色テキストのみ
  - **休息日ヒーロー刷新**: REST DAY 眉 + セリフ大見出し (text-wrap:balance で不格好な折返し根治) + 次開催日を縦罫線仕切りのセリフ数字2列に
  - **バグ修正**: 推奨ログの戦略バッジが英語生キー(value_stack_uren等)のまま横はみ出し → strategies_def の日本語短縮名で表示+折返し
  - **二重見出し整理**: 目次タイトルと内箱タイトルの重複を内側 display:none で解消
  - **実装方法**: styles.css 末尾に「NOIR II — MAISON」ブロック(約500行)を追記 (今後の調整もこのブロックを触る)。app.js は絵文字文字列の置換のみ約30箇所。sw.js v69
  - **検証**: smoke 128/0 / 360〜1280px 横はみ出しゼロ / JSエラーなし(既存のローカル限定404のみ) / 平均明度16.5/255で漆黒キープ / 閉じた状態・開いた状態・モーダル・PC幅まで実機スクショ全段確認 / 本番 keiba-navigator.vercel.app 反映を撮影確認
  - **⚠️ 次セッションへ**: 今後の見た目調整は styles.css 末尾の NOIR II ブロックに後勝ちで追記するのが安全 (上の NOIR OVERRIDE と !important が競合するため順序が重要)
...
- **🏇⚡ Wave36 — 土曜(6/6)本番に向けた総仕上げ (2026-06-04 夜・本人「次の土曜から本気・儲けることが目標・完璧と言えるまで全部仕上げて」指示で就寝中に実施)** —
  ...
  - **③ 最新データ(5/31結果込・78,013ベット)でリークなし再検証**: walk_forward_value_ev / analyze_value_ev / experiment_engine / rankings を全部再実行。**100%超のポケットは依然なし**(市場効率的・正直に維持)。ただし**「単勝・1番人気でAI補正EV1.15以上」が99.5%(3/6期間勝ち・85件)で損益分岐に最接近**と発見 → 実験室に観察作戦2つ(EV1.05/1.15版)を追加し、データが増えるたび自動採点される(実験室の首位 82.1%→99.5%に更新)
  - **④ 10年データ(option=4)を再挑戦**: 今回は即-501ではなくサーバ応答待ちで無通信ハング → 30分観察して打ち切り。引き続き配信されない結論(諦め継続)
  - **⑤ 掃除と修正**: 役目を終えた6/1限定タスク2個を削除 / fetch_diff_hourly.ps1 にUTF-8 BOM付与(cp932誤読事故の予防) / 古いpytest(レコード33種→WF追加で34種)を修正
  - **⑥ 最終QA全緑**: smoke 128/0 / pytest 245 passed / web build成功 / 本番API 6本全200 / 実機スクショで旧アプリ(NOIR漆黒×金・カウントダウン「次は6/6・6/7」)とv2両方の正常描画を目視確認。前セッション宿題のNOIR実機確認も完了
  - **commit**: 9b8d168 (Wave36) / d553b6d (Wave36-B) push済
  - **⚠ 土曜の自動経路まとめ**: GapFill-0700(完全版pipeline)→ 09:00〜20:30毎時(取得+RTオッズ+予想更新+push)。全タスク WakeToRun=True・直近全部0x0

- **🔬 Wave36-C — 「本当に土曜に動くのか」の実走証明 (2026-06-05 深夜・本人「いつも『土曜に行ける』と言って当日できませんでしたのパターンがある。反省して何度も確認してから終わって」叱責を受けて実施)** —
  - **本物のタスクスケジューラ経由でフルコース強制実走**: 取得起点を3日前に巻き戻して `Start-ScheduledTask KeibaDiffFetch-0030` → **結果0x0で完走**をログで確認 (21.3万レコード取得→races 3,660/results 3,569→翌日(土)24R予想→カード→features→再学習→おすすめ→precompute→git push まで一気通貫・所要約3分)
  - **0:30の定時実行も新コードで成功済みを確認**: 土曜24Rを認識しRT取得を試行(未発売=0/24は想定通りの正常拒否)・BOM付与でログの文字化けも解消
  - **実走で発見した本物のバグを修正**: 金曜深夜時点の土曜レースは**枠順確定前の空シェル(48頭重複・馬番0)**で、「全頭2.08%の均等割り」という無意味な予想が生成され本番に流れる隙があった → predict_lightgbm に「全頭の馬番が0のレースは予想しない+古いゴミ予想を削除」ガードを追加し、--date 20260606/07 で24件スキップ+ゴミ削除を実証 (commit f7e17b7)。枠順確定(金曜夕方)後は自動で本物の予想に切り替わる
  - **自動の再検証を2本予約 (リモート検証エージェント・claude.ai/code/routines)**: ①金曜19:45 JST「土曜本番前チェック」(枠順確定・前売りオッズ・予想生成・本番APIを独立検証) ②土曜8:10 JST「本番朝チェック」(GapFill実行・予想+オッズ・カード前進・本番APIを厳しく検証)。どちらも結果を **Gmail下書き** で shougi.hajime@gmail.com に日本語報告・リポジトリ内のバグなら自動修正してpushする権限つき (trig_01TkhMSyGL4FPriePfzskPhh / trig_01VXDkj6c7mmioLbbtMfqPP8)
  - **正直な区別**: 実走で証明済み=フルコース完走・シェルガード・RT取得の正常拒否・push。当日まで確認できないこと=前売りオッズが実際に流れ込む瞬間(金曜18:30頃)と土曜のRT取得成功 → そのために上記の自動再検証を仕掛けた
  - smoke 128/0 / pytest 245 passed 維持
...
- **⏰🐎 「6月1日に確定データが配信されたら自動で空白を埋める」夜間自動化を仕込み (2026-05-31 夜・本人「6/1になったら情報が取れる→取りに行って・空白の時間を全部埋めて・明日朝起きたら全部効いているように」指示)** —
  ...
  - **【6/1 早朝の経過・追記】**:
    - 共有Supabase(クラウド)は **21:40時点で復活確認済**(keiba REST が503→200)。原因だった実在しないスキーマ `shogi_jikanwari` は公開リストから除去済(グローバルCLAUDE.md の教訓参照)。請求月待ちではなくスキーマ修正で復旧していた。全16アプリのログイン/同期OK
    - **重大バグを早朝に発見・修正**: 夜間タスクが 0:30 実行で `0xFFFD0000` 失敗。原因は `register_overnight_gapfill.ps1` を PowerShell 5.1 が BOM無しUTF-8をcp932誤読し **競馬パスが `遶ｶ鬥ｬ` に化けて**いた。→ インライン再登録で正しいパス+作業フォルダ指定に修正、取得タスク/完全版タスクとも**実機で0x0成功を確認**。スクリプトも漢字を文字コード生成する版に直して commit b2d083f。これが無ければ今日ずっと空振りだった
    - **6/1 05:19時点でも、JRA-VANは先週末(5/30・5/31)の確定データをまだ未配信**(option=1で取り直しても「該当データなし」、結果は5/30:1・5/31:5の部分のみ・カード日付20260524のまま)。確定データは6/1の0時ではなく**月曜日中(営業時間)に配信される**もよう。配信され次第 06:30/07:00/09:00〜の各タスクが自動で拾い、カードが前進してpush→本番反映する(本人操作不要)
...
- **🐎❌ NOIR仕上げ — バーチャルな馬の装飾を全廃止 + 文字拡大 (2026-05-31・本人「後ろのバーチャルな馬が冴えない・本物志向で行きたい」「もう少し文字を大きく見やすく」指示)** —
  ...
  - 本文/見出し/買い目/レース行/ボタン/ボトムナビの font-size を一段拡大(app-root 16.5px・理由文16.5px・買い目is-main 28px 等)で可読性UP
  - **本物の写真背景=実装済(当初「保留」としたが訂正)**: 「外部画像CDN通信不可」は誤判断で、`Invoke-WebRequest`/`curl`なら images.unsplash.com からDL可だった。著作権フリー(Unsplash・商用可/帰属不要)の「夕暮れに芝を駆ける白馬」を `assets/horse-bg.jpg` として採用 → `body::before` に最背面固定、黒グラデのベール＋彩度0.6で漆黒×金に馴染ませた。上40%にうっすら白馬が透け下へ完全な黒に溶ける。実機で写真帯21/255・全体34/255=漆黒キープ確認。**commit cb5e974 push 済**。誤コミットした候補画像/スクショ/スキャンメモは掃除済。別セッションのデータ作業ファイル(`_rt.log`/`jvinit.txt`等)は未追跡のまま残置(当方作成でないため触らない)
  - **検証の注意**: 当セッション終盤、headlessスクショ画像のRead返却が一時的に空になる harness 不調が発生。変更はCSS/HTMLのみ(ビルド不要・低リスク)で、直前のNOIRスクショで黒×金描画は確認済のため push 済。次セッションで実機表示を一度目視確認推奨。**commit 7d7c8f8 push 済**

- **🏛️ NOIR — 旧アプリを「真っ黒×細い金」の高級テーマへゼロから全面刷新 (2026-05-31・本人「今の状態が最高に嫌・全部変えたい・真逆の品のいい大人の雰囲気に・何時間かけてもいい」指示。4案から本人が「真っ黒×細い金の高級感」を選択)** —
  - 直前の「炭×シャンパンゴールド」も本人が強く拒否 → 中途半端な手直しをやめ、ゼロから作り直し。漆黒のブランドのような凛とした佇まいを目標に
  - `styles.css` の `:root` トークンを総入れ替え: 地=漆黒 #060606、主役=ヘアライン金 #C9A85C/#E4C77E、緑青紫はごく控えめなセージ/グレー、テキストは暖色の白〜グレー。色相を金一色に寄せ装飾を削ぎ落とす方針
  - 背景の虹色グラデ・ノイズグレイン(`body::before`)を廃止 → 上端に金のひと刷毛だけの漆黒に
  - 既存6452行は触らず、末尾に **NOIR OVERRIDE(16ブロック・約190行)** を追記して上書き: 全要素の backdrop-filter(ガラスぼかし)を無効化・装飾アニメ(浮遊/グロー/明滅/弾み)を停止・全カードをフラットな黒面＋1px罫線に統一・主役カードはヘアライン金縁・大きな数字は金・ボタンは金の輪郭・バッジは塗りをやめて罫線・ボトムナビは漆黒＋上罫線・グラデ文字は金一色に
  - 見出しフォントに上品なセリフ **Cormorant Garamond** を採用(index.html の Google Fonts に追加)。theme-color も #060606
  - **検証**: ローカル起動→実機スクショ。.NET で明るさ計測 = 全体27.4/255 = 漆黒達成・文字くっきり。**commit 9c54f9e push 済 → keiba-navigator.vercel.app に自動反映**
  - **⚠️ 設計メモ**: NOIRの上書きは styles.css 末尾の「NOIR OVERRIDE」ブロックに集約。今後の色/質感の微調整はここと `:root` トークンを触れば全体に効く。本人の見ている画面=旧アプリ(ルート直下 styles.css)

- **🖤 旧アプリ(本人が常用)を「炭×シャンパンゴールド」大人ダークへ刷新 (2026-05-31・本人「カラフルで子供っぽい→大人っぽくおしゃれに・大体作り直して」指示)** —
  - **最重要の気づき**: 本人がずっと見ているのは旧アプリ `keiba-navigator.vercel.app`(バニラJS = ルート直下の `index.html`/`styles.css`/`app.js`)。前回までのデザイン変更は新アプリ v2(`web/`)側にしか入っておらず本人の画面は変わっていなかった。本人が「今の住所をそのまま直す」を選択 → **旧アプリ側の `styles.css` を直接刷新**
  - `styles.css`(6454行)の配色トークンを大人っぽく作り替え: 炭の黒地(#0D0F12)、彩度を落としたシャンパンゴールド(#D9B779)、落ち着いた緑(#4FB98A)、霧がかった青/紫。テキストは暗地で読める明るい白系(#ECEDEF)
  - 白いガラス面(rgba白157箇所)を sed で暗いガラス面へ一括変換。虹色の背景を黒地＋ごく薄い金/緑グローに鎮静。明るいパステルのチップは alpha 0.15 に。暗地で沈む濃い文字色(#0f172a/#047857/#92400e/#6d28d9 等 多数)を perl で明るい上品版へ一括差し替え。朝の概要トースト/最高的中バナーの明るい背景も暗いガラスへ
  - **検証**: ローカル `node server.js` 起動 → 実データ込みで実機スクショ(スマホ420px)。.NET で明るさ計測 = 全体32〜36/255 = 黒地が崩れず文字くっきりを確認。**commit c17c6d1 + 仕上げ commit を push 済 → keiba-navigator.vercel.app に自動反映**
  - **⚠️ 次セッション以降の注意**: 本人の見ている画面=**旧アプリ(ルート直下 `styles.css`)**。見た目変更は原則ここを触る。`web/`(v2)はデザイン別管理で本人は普段見ていない

- **🗒️ 「あったら便利」さらに4点を実装 (2026-05-31・本人が選択肢から4つ全部選択。CSVは取り込みもセットで希望)** —
  - **① 記録をカレンダー表示**: `RecordsHub.tsx`(記録ノート)新規。月カレンダーで勝ち=緑/負け=赤/結果待ち=青を色分け、各日に収支(±Xk)or件数。日タップでその日の馬券一覧、月送り可
  - **② 記録の一覧と編集**: 同ハブの「一覧・編集」タブ。全記録(当/外/待ち)を新しい順に表示し、買い目/金額/結果/払戻を後から直す・消す
  - **③ 記録の書き出し・取り込み (`lib/csv.ts` 新規)**: CSV(UTF-8 BOM=Excelで文字化けしない)で書き出し、CSVから取り込み。取り込みは「追加」で重複ID・内容はスキップ。書き出したCSVをそのまま取り込める(列名/位置どちらも許容・結果は日英両対応)。store.tsに`mergeBets`追加
  - **④ 今日のひとこと助言 (`DailyAdvice.tsx` 新規)**: 画面上部(ヒーロー直下)に記録から自動の短い助言。予算オーバー>直近3連敗>予算8割>3連勝>通常 の優先度で出し分け
  - **配置**: page.tsx に DailyAdvice(上部)+ RecordsHub(成績の見える化の直後)。**検証**: web `npm run build` 成功・型チェック通過。**commit 7d8a3f2 push 済(本番v2へ自動反映)**

- **🧰 「あったら便利」4点を実装 (2026-05-31・本人が選択肢から4つ全部選択)** —
  - **① 手で記録するボタン (`ManualBetModal.tsx` 新規)**: 開催なしの日・過去に買った分も、日付/レース名/券種/買い目/金額/結果を自分で入力し収支・成績に反映。「結果待ちの記録」欄に「手で記録」ボタンを設置(記録ゼロでも空状態に常設)
  - **② WIN5を記録できる枠**: 券種にWIN5を追加 + WIN5欄(日曜)に「買ったWIN5を記録する」ボタン。WIN5は買い目入力を任意に(5レースぶんで長いため)
  - **③ 反省タグを賢く (`MissTagPicker.tsx` 新規)**: 外れにする時、理由(人気を過大評価/ペース想定外れ/馬場/距離 など8種)をタップで複数選べる。PendingBetsListの外れフロー + 手入力の両方で使用。選んだタグが反省ダッシュボードの集計に効く(従来は情報不足で常に「原因特定が難しい」だった)
  - **④ 買う前の最終チェック (`BetConfirmModal.tsx`)**: 正直EV(冷ました現実的な見込み)と「今月あといくら使える→買うと残りいくら」を表示。予算オーバー時は赤く「見送るのも勝つための判断」と警告
  - **検証**: web `npm run build` 成功・型チェック通過。**commit 4154412 push 済(本番v2へ自動反映)**

- **🌃 「黒地ネオンのナイトターフ」テーマへ全面刷新 (2026-05-31・本人「真っ白で物足りない→世界一すごい現代デザインに・インパクト」指示。4案から本人が「ネオン・ナイトターフ」を選択)** —
  - `web/src/app/globals.css` のデザイントークンを丸ごと夜の競馬場へ作り変え: 地は深い黒(#05080B)、主役はネオン緑(#16E07E)とシアン(#2FF0E0)、文字は暗地で映える明るい緑みの白。背景にネオンの光・近未来グリッド線・ネオン緑の走る馬を追加
  - 暗地で読みにくくならないよう一括補正をCSSに追加(緑/金ボタンは暗文字+発光・入力欄/テキストエリアも暗いガラス面・くぼみ面に薄い輪郭)。各部品のクラスは極力触らずトークン経由で夜テーマに自動追従
  - 明るいまま取り残されていた箇所を黒地用に修正: HeroBanner(明るい空グラデ→ネオン夜)・Card gold面・RaceCard本命玉の文字色・StarRating空星・layout の themeColor/colorScheme
  - **検証**: `npm run build` 成功(型チェック通過) / 本番ビルドを起動しスマホ390px・PC1280pxでスクショ撮影 → 黒地にネオンが光り文字もくっきり読めることを確認。**commit 8105532 push 済(本番v2へ自動反映)**
