...
- **🐎❌ NOIR仕上げ — バーチャルな馬の装飾を全廃止 + 文字拡大 (2026-05-31・本人「後ろのバーチャルな馬が冴えない・本物志向で行きたい」「もう少し文字を大きく見やすく」指示)** —
  ...
  - **本物の写真背景は保留**: 本人は選択肢で「本物の写真を背景に」を選んだが、**この開発環境は外部画像CDN(Unsplash/Pexels等)へ通信不可**(DNS解決不可・git/GitHubのみ到達)。写真をローカルに取り込んで暗く敷く処理は次の手として、本人に写真ファイルを置いてもらうか信頼できる配信URLの承認をもらってから実装する。**今回は「冴えないバーチャル馬の除去＋文字拡大」を先行リリース**
