...
### ✅ 直近で済んだこと
- **🏆 Wave29 (2026-05-20 朝・ユーザー指示「5 = 全部 Claude 判断」で V-馬連 222% 革命的発見)** —
  - **Wave29-A 障害競走除外モデル** (`train_lightgbm.py --skip-shougai` 追加):
    - 学習データ 3,377 R のうち障害は 117 件 (3%)
    - 除外して再学習 → BIG -12.6%・TURF +1.8% の混合結果 → 本番投入見送り・旧モデル復帰
  - **🚀 Wave29-B 券種別 × VALUE 戦略 — 革命的発見!** (`value_multi_bet_sweep.py` 新規):
    nopop top1/2/3 で 5 券種 × 4 閾値の Walk-forward 評価
    | 券種 | 閾値 | avg ROI | σ | 勝期間 | 件数 |
    |------|------|---------|-----|--------|------|
    | 複勝 (旧 VALUE) | 0.16 | 152.62% | 37 | 6/7 | 2673 |
    | **馬連 nopop top1-top2** | **0.30** | **165.29%** | 66 | **7/7** ⭐ | 956 |
    | **馬連 nopop top1-top2** | **0.16** | **222.57%** | 92 | 6/7 | 2673 |
    | ワイド 3 点 | 0.16 | 123.14% | 26 | 6/7 | 2673 |
    | 3 連複 ボックス | 0.16 | 124.49% | 57 | 5/7 | 2673 |
    | **3 連単 nopop top1->2->3** | **0.20** | **308.50%** | 191 | 6/7 | 2160 |
    - **本番投入 2 戦略**:
      - **V-馬連** (閾値 0.30): avg **165.29%** / 全期間 100%+ (勝 7/7) ⭐ 最も信頼性高
      - **V-馬連HOT** (閾値 0.16): avg **222.57%** / 件数 2673 / 勝 6/7 ⭐ 最高 ROI
  - **Wave29-C Isotonic Regression Calibration** (`calibrate_isotonic.py` 新規):
    - nopop モデルを Isotonic で校正試行
    - test 期間で過剰補正・LogLoss 悪化 (0.225 → 0.314) → 採用見送り
  - **Wave29-D JV-Link 過去 10 年取得試行**:
    - JVOpen aggregate option=4 fromtime=20140101 → rc=-501 (JRA-VAN サーバ拒否)
    - 朝の自動リトライ (9:00/12:00/15:00) で再試行待ち
  - **本番デプロイ**: commit `b5c6726` push origin main 済

### 🔜 次の一歩 (世界一完成へ・更なる飛躍)
- **3 連単 nopop 戦略 を本番投入** (avg 308%・σ 191 は大きいが魅力的)
- **JV-Link 過去 10 年フル取得** (60 万行 / AUC +3〜5% / JRA-VAN サーバ朝待ち)
- **券種別 × 閾値の完全グリッドサーチ** (馬連 × G1 × ハイペース等の組合せ)
- **Walk-forward 検証期間を 16 期間に増やす** (現在 8 期間・データ多い時に統計的に強くなる)

### ⚡ 最強戦略の進化史
| Wave | 戦略 | avg ROI | 勝期間 | 件数 |
|------|------|---------|--------|------|
| Wave19.8 | BEST (combo_best_and_gap) | 112.1% | 7/7 | 53 |
| Wave27 | VALUE (nopop 複勝 0.16) | 152.62% | 6/7 | 2673 |
| **Wave29** | **V-馬連** (nopop 馬連 0.30) | **165.29%** | **7/7** | 956 |
| **Wave29** | **V-馬連HOT** (nopop 馬連 0.16) | **222.57%** | 6/7 | 2673 |
| (理論) | V-3連単 (nopop 3連単 0.20) | 308.50% | 6/7 | 2160 |
