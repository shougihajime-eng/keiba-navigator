...
- **🔎 KeibaDiffFetch×16本の効率精査 (2026-06-05・本人指示「効率が悪いなら直す・無駄じゃないなら触るな」) → 結論: 無駄ではない・触らない**: ログ実測で検証。①空振り時(新データなし)は**1回約14秒で即終了**(JVOpen rc=-1=データなし正常終了→RT試行→終了) ②新データがある時だけ重い本処理(build+予想+特徴量+学習+push=約4分。今日6/5は11:30の1回だけ) ③モデル再学習は6時間以内ならスキップ済み ④夜中の回(00:30/03:30/06:30)も各14秒=蓄積系の夜間配信の取りこぼし防止として妥当。唯一の微小な無駄=前売り開始前の明日レースへのRT単複オッズ試行24件×約13秒(金曜昼など)だが、前売り開始時刻は日により変わるためスキップ条件を入れると**取り損ねリスク>節約(1日2〜3分のCPU)**で見送り。
