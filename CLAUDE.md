...
推定順位リスト（レース詳細モーダル）を、各馬1行の見やすい表に刷新。各馬に: **能力値バー**・**本命◎/対抗○/単穴▲/連下△の色シグナル**・**信頼度ランクA〜D**・**勝率/2着内/3着内**・人気/オッズ/期待値(EV)。2着内・3着内は全馬の1着予想から **Harville 法** で推定し「参考値」と明記（1着確率しか無いため＝嘘をつかない）。実装は `app.js`（推定順位の描画ブロック＋harvillePlace）と `styles.css`（signal-tag / ability-bar / rate-cell / conf-badge）。smoke 128/0 通過・main へ push 済み＝Vercel 自動デプロイで本番反映確認済み。姉妹アプリ「必殺一号艇」も同日に同基準で刷新済み。
