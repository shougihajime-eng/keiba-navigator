...
- **🔬 6/13 本人「競馬で勝った有名な人たちのシステムを学んで取り込め」→ 調査+2次元補正+信頼区間まで実施 (2026-06-13)** —
  ...
  - **🔜 次の一歩(未実施・本番アプリへの配線は慎重に)**: ①この2次元補正を honest_ev.py の手調整 _COOL_BANDS の代わりに**データ駆動の補正表**として配線(EV表示の精度UP=「買わない判断」の質向上。利益保証ではない)。②事前オッズ限定の検証は既存だが、計算式を honest_ev に正式統合。**レース当日に本番のEV/おすすめを変える変更は、開催日を避けて入れること**(today のおすすめが途中で変わると本人が混乱する)。
